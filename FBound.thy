...
lemma strong_deferred_strong_rows_raw_bridge_rows_empty [simp]:
  "strong_deferred_strong_rows_raw_bridge_rows r [] = {rerase (intern r)}"
  by (simp add: strong_deferred_strong_rows_raw_bridge_rows_def
...
lemma strong_deferred_strong_rows_raw_bridge_rows_subset_least_owner_dag:
  "strong_deferred_strong_rows_raw_bridge_rows r s \<subseteq>
    strong_deferred_strong_rows_raw_least_owner_dag r s"
proof -
  have "strong_deferred_strong_rows_raw_bridge_rows r s \<subseteq>
      strong_deferred_strong_rows_raw_least_owner r s"
    by (rule strong_deferred_strong_rows_raw_bridge_rows_subset_least_owner)
  also have "... \<subseteq>
      strong_deferred_strong_rows_raw_least_owner_dag r s"
    by (rule strong_deferred_strong_rows_raw_least_owner_subset_dag)
  finally show ?thesis .
qed
...
lemma strong_deferred_strong_rows_raw_least_owner_dag_norm_active_suffix_member_sizeI:
  assumes init: "rerase (intern r) \<in> U"
    and flat_closed: "\<And>q. q \<in> U \<Longrightarrow> set (rflts [q]) \<subseteq> U"
    and norm: "\<And>xs c q p. set xs \<subseteq> U \<Longrightarrow>
      q \<in> set xs \<Longrightarrow> p \<in> set (rpder_norm_list c q) \<Longrightarrow>
    ...
    and active_suffix_closed:
      "raw_shared_prune_active_suffix_closure U \<subseteq> U"
    and member_size: "\<And>q. q \<in> U \<Longrightarrow> rsize q \<le> M"
    and p: "p \<in> strong_deferred_strong_rows_raw_least_owner_dag r s"
  shows "rsize p \<le> M"
proof (rule strong_deferred_strong_rows_raw_least_owner_dag_member_sizeI
    [OF _ p])
  fix q
  assume q: "q \<in> strong_deferred_strong_rows_raw_bridge_rows r s"
  show "rsize q \<le> M"
    by (rule
        strong_deferred_strong_rows_raw_bridge_rows_norm_active_suffix_member_sizeI
        [OF init flat_closed norm active_suffix_closed member_size q])
qed
...
lemma strong_deferred_final_active_suffix_payload_key_dag_universe_subset_closed_root_universe:
  assumes roots:
      "strong_deferred_final_active_suffix_payload_roots r s \<union>
       strong_deferred_final_active_suffix_keys r s \<subseteq> U"
    and subterm_closed: "\<And>q. q \<in> U \<Longrightarrow> rsubterms q \<subseteq> U"
  shows "strong_deferred_final_active_suffix_payload_dag_universe r s \<union>
...
proof -
  ...
  have keys: "strong_deferred_final_active_suffix_keys r s \<subseteq> U"
    using roots by blast
  have payload_dag:
...
lemma strong_deferred_final_active_suffix_decomp_bound_shared_root_boundI:
  assumes rows:
      "card (strong_deferred_final_active_suffix_rows r s) \<le> R"
    and roots:
      "strong_deferred_final_active_suffix_payload_roots r s \<union>
...
proof -
  ...
  have payload:
      "card (strong_deferred_final_active_suffix_payload_dag_universe r s)
        \<le> D"
  proof -
    have "card (strong_deferred_final_active_suffix_payload_dag_universe r s)
        \<le> card U"
      by (rule card_mono[OF finite payload_subset])
    also have "... \<le> D"
      by (rule card_bound)
    finally show ?thesis .
  qed
  have keys:
      "card (strong_deferred_final_active_suffix_key_dag_universe r s)
        \<le> D"
  proof -
    have "card (strong_deferred_final_active_suffix_key_dag_universe r s)
        \<le> card U"
      by (rule card_mono[OF finite key_subset])
    also have "... \<le> D"
      by (rule card_bound)
    finally show ?thesis .
  qed
...
lemma strong_deferred_final_active_suffix_decomp_bound_shared_root_linearI:
  assumes rows:
      "card (strong_deferred_final_active_suffix_rows r s) \<le>
        R * rxsize r"
    and roots:
...
lemma strong_deferred_final_active_suffix_row_dag_universe_bridge_ownerI:
  assumes rows:
...
lemma card_strong_deferred_final_active_suffix_rows_bucket_linearI:
  assumes keys_bound:
      "card (strong_deferred_final_active_suffix_keys r s) \<le>
        K * rxsize r"
    and bucket_bound:
...
lemma strong_deferred_final_active_suffix_rows_subset_row_dag_universe:
  "strong_deferred_final_active_suffix_rows r s \<subseteq>
    strong_deferred_final_active_suffix_row_dag_universe r s"
...
lemma strong_deferred_final_active_suffix_rows_bound_by_row_dag_universeI:
  assumes "card (strong_deferred_final_active_suffix_row_dag_universe r s) \<le> D"
  shows "card (strong_deferred_final_active_suffix_rows r s) \<le> D"
proof -
  have "card (strong_deferred_final_active_suffix_rows r s) \<le>
      card (strong_deferred_final_active_suffix_row_dag_universe r s)"
    by (rule card_strong_deferred_final_active_suffix_rows_le_row_dag_universe)
  also have "... \<le> D"
    by (rule assms)
  finally show ?thesis .
qed
...
lemma strong_deferred_final_active_suffix_pair_budget_bucket_linearI:
  assumes keys_bound:
    "card (strong_deferred_final_active_suffix_keys r s) \<le>
      K * rxsize r"
    and bucket_bound: "\<And>k. k \<in>
  ...
  shows "strong_deferred_final_active_suffix_pair_budget r s \<le>
    (K * B * B) * rxsize r"
proof -
  have "strong_deferred_final_active_suffix_pair_budget r s \<le>
      (K * rxsize r) * B * B"
    by (rule strong_deferred_final_active_suffix_pair_budget_bucket_bound
        [OF keys_bound bucket_bound])
  also have "... = (K * B * B) * rxsize r"
    by (simp add: algebra_simps)
  finally show ?thesis .
qed
...
lemma strong_deferred_original_final_active_shared_row_dag_linear_contract:
  assumes legacy: "legacy_rexp r"
    and rows_bound:
      "card (strong_deferred_final_active_suffix_rows r s) \<le> rxsize r"
    and shared:
  ...
  shows "((if bnullable (bders_simpStrong (intern r) s)
    ...
    and "card (strong_deferred_final_active_suffix_row_dag_universe r s) \<le>
      (C + 2) * rxsize r"
    and "\<And>q. q \<in> strong_deferred_final_active_suffix_rows r s \<Longrightarrow>
      card (rsubterms q) \<le> (C + 2) * rxsize r"
    and "card (rexp_span_states r s) + card (rexp_span_posix_states r s) \<le>
      2 * rxsize r * Suc (length s) * Suc (length s)"
    and "card (rexp_span_all_split_probes r s) \<le>
      rxsize r * Suc (length s) * Suc (length s) * Suc (length s)"
proof -
  have row_dag_pre:
    "card (strong_deferred_final_active_suffix_row_dag_universe r s) \<le>
      2 * rxsize r + C * rxsize r"
    by (rule
        card_strong_deferred_final_active_suffix_row_dag_universe_shared_component_boundI
        [OF rows_bound shared finite card_bound])
  have row_dag_bound:
    "card (strong_deferred_final_active_suffix_row_dag_universe r s) \<le>
      (C + 2) * rxsize r"
  proof -
    have "2 * rxsize r + C * rxsize r = (C + 2) * rxsize r"
      by (simp add: algebra_simps)
    then show ?thesis
      using row_dag_pre by simp
  qed
  show "((if bnullable (bders_simpStrong (intern r) s)
      then Some (THE v. strong_deferred_span_value r s v)
      else None) = Some v) \<longleftrightarrow> s \<in> r \<rightarrow> v"
  ...
  show "\<And>q. q \<in>
      ...
      legacy_rrexp q"
    by (rule
        strong_deferred_original_final_active_row_dag_linear_contract(5)
        [OF legacy rows_bound row_dag_bound])
  show "card (strong_deferred_final_active_suffix_rows r s) \<le> rxsize r"
    by (rule
        strong_deferred_original_final_active_row_dag_linear_contract(6)
        [OF legacy rows_bound row_dag_bound])
  show "strong_deferred_final_active_suffix_pair_budget r s \<le>
      rxsize r * rxsize r"
    by (rule
  ...
  show "card (strong_deferred_final_active_suffix_row_dag_universe r s) \<le>
      (C + 2) * rxsize r"
    by (rule
        strong_deferred_original_final_active_row_dag_linear_contract(8)
        [OF legacy rows_bound row_dag_bound])
  show "\<And>q. q \<in> strong_deferred_final_active_suffix_rows r s \<Longrightarrow>
      card (rsubterms q) \<le> (C + 2) * rxsize r"
    by (rule
...
lemma strong_deferred_original_final_active_shared_root_universe_linear_contract:
  assumes legacy: "legacy_rexp r"
    and rows_bound:
      "card (strong_deferred_final_active_suffix_rows r s) \<le> rxsize r"
    and roots:
...
proof -
  ...
  show "((if bnullable (bders_simpStrong (intern r) s)
    ...
    by (rule
        strong_deferred_original_final_active_shared_row_dag_linear_contract(1)
        [OF legacy rows_bound shared finite card_bound])
  show "((if bnullable (bders_simpStrong (intern r) s)
      then Some (THE v. strong_deferred_span_value r s v)
      else None) = None) \<longleftrightarrow> \<not> (\<exists>v. s \<in> r \<rightarrow> v)"
    by (rule
        strong_deferred_original_final_active_shared_row_dag_linear_contract(2)
        [OF legacy rows_bound shared finite card_bound])
  show "((if bnullable (bders_simpStrong (intern r) s)
      then Some (THE v. strong_deferred_span_value r s v)
      else None) = Some v) \<Longrightarrow> flat v = s"
    by (rule
        strong_deferred_original_final_active_shared_row_dag_linear_contract(3)
        [OF legacy rows_bound shared finite card_bound])
  show "legacy_rrexp (strong_deferred_final_raw r s)"
    by (rule
        strong_deferred_original_final_active_shared_row_dag_linear_contract(4)
        [OF legacy rows_bound shared finite card_bound])
  show "\<And>q. q \<in>
      strong_deferred_final_active_suffix_row_dag_universe r s \<Longrightarrow>
      legacy_rrexp q"
    by (rule
        strong_deferred_original_final_active_shared_row_dag_linear_contract(5)
        [OF legacy rows_bound shared finite card_bound])
  show "card (strong_deferred_final_active_suffix_rows r s) \<le> rxsize r"
    by (rule
        strong_deferred_original_final_active_shared_row_dag_linear_contract(6)
        [OF legacy rows_bound shared finite card_bound])
  show "strong_deferred_final_active_suffix_pair_budget r s \<le>
      rxsize r * rxsize r"
    by (rule
        strong_deferred_original_final_active_shared_row_dag_linear_contract(7)
        [OF legacy rows_bound shared finite card_bound])
  show "card (strong_deferred_final_active_suffix_row_dag_universe r s) \<le>
      (C + 2) * rxsize r"
    by (rule
        strong_deferred_original_final_active_shared_row_dag_linear_contract(8)
        [OF legacy rows_bound shared finite card_bound])
  show "\<And>q. q \<in> strong_deferred_final_active_suffix_rows r s \<Longrightarrow>
      card (rsubterms q) \<le> (C + 2) * rxsize r"
    by (rule
        strong_deferred_original_final_active_shared_row_dag_linear_contract(9)
        [OF legacy rows_bound shared finite card_bound])
  show "card (rexp_span_states r s) + card (rexp_span_posix_states r s) \<le>
      2 * rxsize r * Suc (length s) * Suc (length s)"
    by (rule
        strong_deferred_original_final_active_shared_row_dag_linear_contract(10)
        [OF legacy rows_bound shared finite card_bound])
  show "card (rexp_span_all_split_probes r s) \<le>
      rxsize r * Suc (length s) * Suc (length s) * Suc (length s)"
    by (rule
        strong_deferred_original_final_active_shared_row_dag_linear_contract(11)
        [OF legacy rows_bound shared finite card_bound])
qed
...
lemma strong_deferred_original_final_active_rowU_dag_owner_contract:
  assumes legacy: "legacy_rexp r"
    and rows_subset:
      "strong_deferred_final_active_suffix_rows r s \<subseteq> RowU"
    and finite_rows: "finite RowU"
    ...
    and dag_subset:
      "strong_deferred_final_active_suffix_row_dag_universe r s \<subseteq> DagU"
    and finite_dag: "finite DagU"
    and dag_card: "card DagU \<le> D"
  shows "((if bnullable (bders_simpStrong (intern r) s)
      then Some (THE v. strong_deferred_span_value r s v)
      else None) = Some v) \<longleftrightarrow> s \<in> r \<rightarrow> v"
    and "((if bnullable (bders_simpStrong (intern r) s)
    ...
    and "strong_deferred_final_active_suffix_payload_roots r s \<union>
      strong_deferred_final_active_suffix_keys r s \<subseteq> DagU"
    and "strong_deferred_final_active_suffix_payload_dag_universe r s \<union>
      strong_deferred_final_active_suffix_key_dag_universe r s \<subseteq> DagU"
    and "card (strong_deferred_final_active_suffix_rows r s) \<le> R"
    and "strong_deferred_final_active_suffix_pair_budget r s \<le> R * R"
    and "card (strong_deferred_final_active_suffix_row_dag_universe r s) \<le> D"
    and "strong_deferred_final_active_suffix_max_row_dag r s \<le> D"
...
proof -
  ...
  proof -
    have "card (strong_deferred_final_active_suffix_row_dag_universe r s) \<le>
        card DagU"
      by (rule card_mono[OF finite_dag dag_subset])
    also have "... \<le> D"
      by (rule dag_card)
    finally show ?thesis .
  qed
  ...
  show "strong_deferred_final_active_suffix_payload_roots r s \<union>
      strong_deferred_final_active_suffix_keys r s \<subseteq> DagU"
    using strong_deferred_final_active_suffix_roots_keys_subset_row_dag_universe
      dag_subset by blast
  show "strong_deferred_final_active_suffix_payload_dag_universe r s \<union>
      strong_deferred_final_active_suffix_key_dag_universe r s \<subseteq> DagU"
    using strong_deferred_final_active_suffix_payload_key_dag_universe_subset_row_dag_universe
      dag_subset by blast
  show "card (strong_deferred_final_active_suffix_rows r s) \<le> R"
    by (rule
        strong_deferred_original_final_active_row_dag_universe_metrics_contract(6)
...
lemma strong_deferred_original_final_active_single_row_dag_universe_contract:
  ...
  shows "((if bnullable (bders_simpStrong (intern r) s)
    ...
    and "strong_deferred_final_active_suffix_row_dag_universe r s \<subseteq>
      rsubterm_closure (strong_deferred_final_active_suffix_rows r s)"
    and "card (strong_deferred_final_active_suffix_rows r s) \<le> D"
    and "strong_deferred_final_active_suffix_pair_budget r s \<le> D * D"
    and "card (strong_deferred_final_active_suffix_row_dag_universe r s) \<le> D"
    and "strong_deferred_final_active_suffix_max_row_dag r s \<le> D"
    and "\<And>q. q \<in> strong_deferred_final_active_suffix_rows r s \<Longrightarrow>
      card (rsubterms q) \<le> D"
...
proof -
  have rows_bound:
    "card (strong_deferred_final_active_suffix_rows r s) \<le> D"
    by (rule strong_deferred_final_active_suffix_rows_bound_by_row_dag_universeI
        [OF row_dag_universe_bound])
  show "((if bnullable (bders_simpStrong (intern r) s)
      then Some (THE v. strong_deferred_span_value r s v)
      else None) = Some v) \<longleftrightarrow> s \<in> r \<rightarrow> v"
  ...
  show "strong_deferred_final_active_suffix_row_dag_universe r s \<subseteq>
    ...
    by (rule
        strong_deferred_original_final_active_row_dag_universe_metrics_contract(5)
        [OF legacy rows_bound row_dag_universe_bound])
  show "card (strong_deferred_final_active_suffix_rows r s) \<le> D"
    by (rule
        strong_deferred_original_final_active_row_dag_universe_metrics_contract(6)
        [OF legacy rows_bound row_dag_universe_bound])
  show "strong_deferred_final_active_suffix_pair_budget r s \<le> D * D"
    by (rule
        strong_deferred_original_final_active_row_dag_universe_metrics_contract(7)
        [OF legacy rows_bound row_dag_universe_bound])
  show "card (strong_deferred_final_active_suffix_row_dag_universe r s) \<le> D"
...
lemma strong_deferred_memo_lexer_final_active_dag_owner_contract:
  assumes legacy: "legacy_rexp r"
    and dag_subset:
      "strong_deferred_final_active_suffix_row_dag_universe r s \<subseteq> DagU"
    and finite_dag: "finite DagU"
...
proof -
  ...
  qed
  have roots_keys_subset:
    "strong_deferred_final_active_suffix_payload_roots r s \<union>
      strong_deferred_final_active_suffix_keys r s \<subseteq>
      strong_deferred_final_active_suffix_row_dag_universe r s"
    by (rule strong_deferred_final_active_suffix_roots_keys_subset_row_dag_universe)
  have payload_key_dag_subset:
    "strong_deferred_final_active_suffix_payload_dag_universe r s \<union>
      strong_deferred_final_active_suffix_key_dag_universe r s \<subseteq>
      strong_deferred_final_active_suffix_row_dag_universe r s"
    by (rule
  ...
  show "strong_deferred_final_active_suffix_payload_roots r s \<union>
      strong_deferred_final_active_suffix_keys r s \<subseteq> DagU"
    by (rule subset_trans[OF roots_keys_subset dag_subset])
  show "strong_deferred_final_active_suffix_payload_dag_universe r s \<union>
      strong_deferred_final_active_suffix_key_dag_universe r s \<subseteq> DagU"
    by (rule subset_trans[OF payload_key_dag_subset dag_subset])
  show "strong_deferred_final_active_suffix_row_dag_universe r s \<subseteq>
      rsubterm_closure (strong_deferred_final_active_suffix_rows r s)"
    by (rule
        strong_deferred_original_final_active_single_row_dag_universe_contract(5)
        [OF legacy row_dag_bound])
  show "card (strong_deferred_final_active_suffix_rows r s) \<le> D"
    by (rule
        strong_deferred_original_final_active_single_row_dag_universe_contract(6)
        [OF legacy row_dag_bound])
  show "strong_deferred_final_active_suffix_pair_budget r s \<le> D * D"
    by (rule
        strong_deferred_original_final_active_single_row_dag_universe_contract(7)
        [OF legacy row_dag_bound])
  show "card (strong_deferred_final_active_suffix_row_dag_universe r s) \<le> D"
    by (rule row_dag_bound)
  show "strong_deferred_final_active_suffix_max_row_dag r s \<le> D"
    by (rule
        strong_deferred_original_final_active_single_row_dag_universe_contract(9)
        [OF legacy row_dag_bound])
  show "\<And>q. q \<in> strong_deferred_final_active_suffix_rows r s \<Longrightarrow>
      card (rsubterms q) \<le> D"
    by (rule
        strong_deferred_original_final_active_single_row_dag_universe_contract(10)
        [OF legacy row_dag_bound])
  show "card (rexp_span_states r s) + card (rexp_span_posix_states r s) \<le>
      2 * rxsize r * Suc (length s) * Suc (length s)"
    by (rule
...
lemma strong_deferred_memo_lexer_final_active_row_dag_linear_contract:
  assumes legacy: "legacy_rexp r"
    and row_dag_universe_bound:
      "card (strong_deferred_final_active_suffix_row_dag_universe r s) \<le>
        K * rxsize r"
  shows "(strong_deferred_memo_lexer r s = Some v) \<longleftrightarrow> s \<in> r \<rightarrow> v"
    ...
    and "card (rexp_span_all_split_probes r s) \<le>
      rxsize r * Suc (length s) * Suc (length s) * Suc (length s)"
proof -
  let ?DagU = "strong_deferred_final_active_suffix_row_dag_universe r s"
  have dag_subset: "?DagU \<subseteq> ?DagU"
    by (rule subset_refl)
  have finite_dag: "finite ?DagU"
    by simp
  show "(strong_deferred_memo_lexer r s = Some v) \<longleftrightarrow> s \<in> r \<rightarrow> v"
    by (rule
        strong_deferred_memo_lexer_final_active_dag_owner_contract(1)
        [OF legacy dag_subset finite_dag row_dag_universe_bound])
  show "(strong_deferred_memo_lexer r s = None) \<longleftrightarrow>
      \<not> (\<exists>v. s \<in> r \<rightarrow> v)"
    by (rule
        strong_deferred_memo_lexer_final_active_dag_owner_contract(2)
        [OF legacy dag_subset finite_dag row_dag_universe_bound])
  show "strong_deferred_memo_lexer r s = lexer r s"
    by (rule
        strong_deferred_memo_lexer_final_active_dag_owner_contract(3)
        [OF legacy dag_subset finite_dag row_dag_universe_bound])
  show "strong_deferred_memo_lexer r s = Some v \<Longrightarrow> flat v = s"
    by (rule
        strong_deferred_memo_lexer_final_active_dag_owner_contract(4)
        [OF legacy dag_subset finite_dag row_dag_universe_bound])
  show "legacy_rrexp (strong_deferred_final_raw r s)"
    by (rule
        strong_deferred_memo_lexer_final_active_dag_owner_contract(5)
        [OF legacy dag_subset finite_dag row_dag_universe_bound])
  show "\<And>q. q \<in>
      strong_deferred_final_active_suffix_row_dag_universe r s \<Longrightarrow>
      legacy_rrexp q"
    by (rule
        strong_deferred_memo_lexer_final_active_dag_owner_contract(6)
        [OF legacy dag_subset finite_dag row_dag_universe_bound])
  show "strong_deferred_final_active_suffix_payload_roots r s \<union>
      strong_deferred_final_active_suffix_keys r s \<subseteq>
      strong_deferred_final_active_suffix_row_dag_universe r s"
    by (rule
        strong_deferred_memo_lexer_final_active_dag_owner_contract(8)
        [OF legacy dag_subset finite_dag row_dag_universe_bound])
  show "strong_deferred_final_active_suffix_payload_dag_universe r s \<union>
      strong_deferred_final_active_suffix_key_dag_universe r s \<subseteq>
      strong_deferred_final_active_suffix_row_dag_universe r s"
    by (rule
        strong_deferred_memo_lexer_final_active_dag_owner_contract(9)
        [OF legacy dag_subset finite_dag row_dag_universe_bound])
  show "strong_deferred_final_active_suffix_row_dag_universe r s \<subseteq>
      rsubterm_closure (strong_deferred_final_active_suffix_rows r s)"
    by (rule
        strong_deferred_memo_lexer_final_active_dag_owner_contract(10)
        [OF legacy dag_subset finite_dag row_dag_universe_bound])
  show "card (strong_deferred_final_active_suffix_rows r s) \<le>
      K * rxsize r"
    by (rule
        strong_deferred_memo_lexer_final_active_dag_owner_contract(11)
        [OF legacy dag_subset finite_dag row_dag_universe_bound])
  show "strong_deferred_final_active_suffix_pair_budget r s \<le>
      (K * rxsize r) * (K * rxsize r)"
    by (rule
        strong_deferred_memo_lexer_final_active_dag_owner_contract(12)
        [OF legacy dag_subset finite_dag row_dag_universe_bound])
  show "card (strong_deferred_final_active_suffix_row_dag_universe r s) \<le>
      K * rxsize r"
    by (rule row_dag_universe_bound)
  show "strong_deferred_final_active_suffix_max_row_dag r s \<le>
      K * rxsize r"
    by (rule
        strong_deferred_memo_lexer_final_active_dag_owner_contract(14)
        [OF legacy dag_subset finite_dag row_dag_universe_bound])
  show "\<And>q. q \<in> strong_deferred_final_active_suffix_rows r s \<Longrightarrow>
      card (rsubterms q) \<le> K * rxsize r"
    by (rule
        strong_deferred_memo_lexer_final_active_dag_owner_contract(15)
        [OF legacy dag_subset finite_dag row_dag_universe_bound])
  show "card (rexp_span_states r s) + card (rexp_span_posix_states r s) \<le>
      2 * rxsize r * Suc (length s) * Suc (length s)"
    by (rule
        strong_deferred_memo_lexer_final_active_dag_owner_contract(16)
        [OF legacy dag_subset finite_dag row_dag_universe_bound])
  show "card (rexp_span_all_split_probes r s) \<le>
      rxsize r * Suc (length s) * Suc (length s) * Suc (length s)"
    by (rule
...
lemma strong_deferred_memo_lexer_strong_rows_bridge_owner_linear_contract:
  assumes legacy: "legacy_rexp r"
    and bridge_subset:
      "strong_deferred_final_active_suffix_row_dag_universe r s \<subseteq>
        strong_deferred_strong_rows_raw_bridge_owner r s"
...
lemma strong_deferred_memo_lexer_final_active_component_union_closed_root_linear_contract:
  assumes legacy: "legacy_rexp r"
    and rows:
      "card (strong_deferred_final_active_suffix_rows r s) \<le>
        R * rxsize r"
...
proof -
  ...
  show "(strong_deferred_memo_lexer r s = Some v) \<longleftrightarrow> s \<in> r \<rightarrow> v"
    by (rule
        strong_deferred_memo_lexer_final_active_component_union_bound_contract(1)
        [OF legacy component_bound])
  show "(strong_deferred_memo_lexer r s = None) \<longleftrightarrow>
      \<not> (\<exists>v. s \<in> r \<rightarrow> v)"
    by (rule
        strong_deferred_memo_lexer_final_active_component_union_bound_contract(2)
        [OF legacy component_bound])
  show "strong_deferred_memo_lexer r s = lexer r s"
    by (rule
        strong_deferred_memo_lexer_final_active_component_union_bound_contract(3)
        [OF legacy component_bound])
  show "strong_deferred_memo_lexer r s = Some v \<Longrightarrow> flat v = s"
    by (rule
        strong_deferred_memo_lexer_final_active_component_union_bound_contract(4)
        [OF legacy component_bound])
  show "card (strong_deferred_final_active_suffix_component_union r s) \<le>
      (2 * R + C) * rxsize r"
  ...
  show "card (strong_deferred_final_active_suffix_row_dag_universe r s) \<le>
      (2 * R + C) * rxsize r"
    by (rule
        strong_deferred_memo_lexer_final_active_component_union_bound_contract(6)
        [OF legacy component_bound])
qed

lemma strong_deferred_memo_lexer_final_active_bucket_component_union_contract:
  assumes legacy: "legacy_rexp r"
    and keys_bound:
      "card (strong_deferred_final_active_suffix_keys r s) \<le>
        K * rxsize r"
...
proof -
  have rows:
    "card (strong_deferred_final_active_suffix_rows r s) \<le>
      (K * B) * rxsize r"
    by (rule card_strong_deferred_final_active_suffix_rows_bucket_linearI
        [OF keys_bound bucket_bound])
  show "(strong_deferred_memo_lexer r s = Some v) \<longleftrightarrow> s \<in> r \<rightarrow> v"
    by (rule
        strong_deferred_memo_lexer_final_active_component_union_closed_root_linear_contract(1)
        [OF legacy rows roots subterm_closed finite card_bound])
  show "(strong_deferred_memo_lexer r s = None) \<longleftrightarrow>
      \<not> (\<exists>v. s \<in> r \<rightarrow> v)"
    by (rule
        strong_deferred_memo_lexer_final_active_component_union_closed_root_linear_contract(2)
        [OF legacy rows roots subterm_closed finite card_bound])
  show "strong_deferred_memo_lexer r s = lexer r s"
    by (rule
        strong_deferred_memo_lexer_final_active_component_union_closed_root_linear_contract(3)
        [OF legacy rows roots subterm_closed finite card_bound])
  show "strong_deferred_memo_lexer r s = Some v \<Longrightarrow> flat v = s"
    by (rule
        strong_deferred_memo_lexer_final_active_component_union_closed_root_linear_contract(4)
        [OF legacy rows roots subterm_closed finite card_bound])
  show "card (strong_deferred_final_active_suffix_rows r s) \<le>
      (K * B) * rxsize r"
    by (rule rows)
  show "card (strong_deferred_final_active_suffix_component_union r s) \<le>
      (2 * (K * B) + C) * rxsize r"
    by (rule
        strong_deferred_memo_lexer_final_active_component_union_closed_root_linear_contract(5)
        [OF legacy rows roots subterm_closed finite card_bound])
  show "card (strong_deferred_final_active_suffix_row_dag_universe r s) \<le>
      (2 * (K * B) + C) * rxsize r"
    by (rule
...
proof -
  ...
  show "card (strong_deferred_final_active_suffix_row_dag_universe r s) \<le>
      K * rxsize r"
    by (rule
        strong_deferred_original_final_active_single_row_dag_universe_contract(8)
        [OF legacy row_dag_universe_bound])
  show "strong_deferred_final_active_suffix_max_row_dag r s \<le>
      K * rxsize r"
    by (rule
...
lemma map_rerase_bpders_strong1_rows_intern_subset_least_owner_dag:
  "set (map rerase (bpders_strong1_rows (intern r) s)) \<subseteq>
    strong_deferred_strong_rows_raw_least_owner_dag r s"
proof -
  have "set (map rerase (bpders_strong1_rows (intern r) s)) =
      strong_deferred_strong_rows_raw_bridge_rows r s"
    by (simp add: map_rerase_bpders_strong1_rows_raw
        strong_deferred_strong_rows_raw_bridge_rows_def)
  also have "... \<subseteq>
      strong_deferred_strong_rows_raw_least_owner_dag r s"
    by (rule strong_deferred_strong_rows_raw_bridge_rows_subset_least_owner_dag)
  finally show ?thesis .
qed

lemma asizes_bpders_strong1_rows_intern_least_owner_dag_boundI:
  assumes finite:
    ...
    and member_size:
      "\<And>q. q \<in> strong_deferred_strong_rows_raw_least_owner_dag r s \<Longrightarrow>
        rsize q \<le> M"
  shows "asizes (bpders_strong1_rows (intern r) s) \<le>
    card (strong_deferred_strong_rows_raw_least_owner_dag r s) * M"
proof (rule asizes_distinct_rerase_finite_universe_bound)
  show "finite (strong_deferred_strong_rows_raw_least_owner_dag r s)"
    by (rule finite)
  show "set (map rerase (bpders_strong1_rows (intern r) s)) \<subseteq>
      strong_deferred_strong_rows_raw_least_owner_dag r s"
    by (rule map_rerase_bpders_strong1_rows_intern_subset_least_owner_dag)
  show "distinct (map rerase (bpders_strong1_rows (intern r) s))"
    by (simp add: map_rerase_bpders_strong1_rows_raw)
  show "\<And>q. q \<in> strong_deferred_strong_rows_raw_least_owner_dag r s \<Longrightarrow>
      rsize q \<le> M"
    by (rule member_size)
qed

lemma rsizes_rpders_strong1_rows_raw_intern_least_owner_dag_boundI:
  assumes finite:
      "finite (strong_deferred_strong_rows_raw_least_owner_dag r s)"
    and member_size:
      "\<And>q. q \<in> strong_deferred_strong_rows_raw_least_owner_dag r s \<Longrightarrow>
        rsize q \<le> M"
  shows "rsizes (rpders_strong1_rows_raw (rerase (intern r)) s) \<le>
    card (strong_deferred_strong_rows_raw_least_owner_dag r s) * M"
proof (rule rsizes_distinct_finite_universe_bound)
  show "finite (strong_deferred_strong_rows_raw_least_owner_dag r s)"
    by (rule finite)
  show "set (rpders_strong1_rows_raw (rerase (intern r)) s) \<subseteq>
      strong_deferred_strong_rows_raw_least_owner_dag r s"
  proof -
    have "set (rpders_strong1_rows_raw (rerase (intern r)) s) =
        strong_deferred_strong_rows_raw_bridge_rows r s"
      by (simp add: strong_deferred_strong_rows_raw_bridge_rows_def)
    also have "... \<subseteq>
        strong_deferred_strong_rows_raw_least_owner_dag r s"
  ...
  qed
  show "distinct (rpders_strong1_rows_raw (rerase (intern r)) s)"
    by simp
  show "\<And>q. q \<in> strong_deferred_strong_rows_raw_least_owner_dag r s \<Longrightarrow>
      rsize q \<le> M"
    by (rule member_size)
...
proof -
  show "((if (\<exists>p \<in> set (bpders_strong1_rows (intern r) s). bnullable p)
    ...
    by (rule strong_deferred_row_gate_memo_POSIX_correctness(1)[OF legacy])
  show "((if (\<exists>p \<in> set (bpders_strong1_rows (intern r) s). bnullable p)
      then Some (THE v. strong_deferred_span_value r s v)
      else None) = None) \<longleftrightarrow> \<not> (\<exists>v. s \<in> r \<rightarrow> v)"
    by (rule strong_deferred_row_gate_memo_POSIX_correctness(2)[OF legacy])
  show "((if (\<exists>p \<in> set (bpders_strong1_rows (intern r) s). bnullable p)
      then Some (THE v. strong_deferred_span_value r s v)
      else None) = Some v) \<Longrightarrow> flat v = s"
    by (rule strong_deferred_row_gate_memo_POSIX_flat[OF legacy])
  have asizes_bound:
    "asizes (bpders_strong1_rows (intern r) s) \<le>
      card (strong_deferred_strong_rows_raw_least_owner_dag r s) * M"
    by (rule asizes_bpders_strong1_rows_intern_least_owner_dag_boundI
  ...
  show "asizes (bpders_strong1_rows (intern r) s) \<le> B"
    using asizes_bound card_times cubic by linarith
  have rsizes_bound:
  ...
  show "rsizes (rpders_strong1_rows_raw (rerase (intern r)) s) \<le> B"
    using rsizes_bound card_times cubic by linarith
...
lemma strong_deferred_row_gate_norm_active_suffix_universe_POSIX_contract:
  assumes legacy: "legacy_rexp r"
...
proof -
  ...
  show "((if (\<exists>p \<in> set (bpders_strong1_rows (intern r) s). bnullable p)
    ...
    by (rule strong_deferred_row_gate_least_owner_dag_rows_member_POSIX_contract(1)
        [OF legacy dag_card rows_member cubic])
  show "((if (\<exists>p \<in> set (bpders_strong1_rows (intern r) s). bnullable p)
      then Some (THE v. strong_deferred_span_value r s v)
      else None) = None) \<longleftrightarrow> \<not> (\<exists>v. s \<in> r \<rightarrow> v)"
    by (rule strong_deferred_row_gate_least_owner_dag_rows_member_POSIX_contract(2)
        [OF legacy dag_card rows_member cubic])
  show "((if (\<exists>p \<in> set (bpders_strong1_rows (intern r) s). bnullable p)
      then Some (THE v. strong_deferred_span_value r s v)
      else None) = Some v) \<Longrightarrow> flat v = s"
    by (rule strong_deferred_row_gate_least_owner_dag_rows_member_POSIX_contract(3)
        [OF legacy dag_card rows_member cubic])
  show "asizes (bpders_strong1_rows (intern r) s) \<le> B"
    by (rule strong_deferred_row_gate_least_owner_dag_rows_member_POSIX_contract(4)
        [OF legacy dag_card rows_member cubic])
  show "rsizes (rpders_strong1_rows_raw (rerase (intern r)) s) \<le> B"
    by (rule strong_deferred_row_gate_least_owner_dag_rows_member_POSIX_contract(5)
        [OF legacy dag_card rows_member cubic])
  show "map rerase (bpders_strong1_rows (intern r) s) =
      rpders_strong1_rows_raw (rerase (intern r)) s"
    by (rule strong_deferred_row_gate_least_owner_dag_rows_member_POSIX_contract(6)
        [OF legacy dag_card rows_member cubic])
  show "set (map rerase (bpders_strong1_rows (intern r) s)) \<subseteq>
      strong_deferred_strong_rows_raw_least_owner_dag r s"
    by (rule strong_deferred_row_gate_least_owner_dag_rows_member_POSIX_contract(7)
        [OF legacy dag_card rows_member cubic])
  show "raw_final_active_suffix_row_dag_universe
        (rsimpStrong_ALTs_raw
          (rpders_strong1_rows_raw (rerase (intern r)) s))
...
lemma strong_deferred_original_raw_row_norm_later_shared_memo_cubic_interface:
  assumes legacy: "legacy_rexp r"
      and init: "rerase (intern r) \<in> U"
      and flat_closed: "\<And>q. q \<in> U \<Longrightarrow> set (rflts [q]) \<subseteq> U"
      and norm: "\<And>xs c q p. set xs \<subseteq> U \<Longrightarrow>
...
proof -
  show "legacy_rrexp (rerase (bders_simpStrong (intern r) s))"
    by (rule strong_deferred_original_memo_budget(1)[OF legacy])
  show "asizes (bpders_strong1_rows (intern r) s) \<le> B"
    by (rule strong_deferred_original_raw_row_norm_later_shared_cubic_universe_interface
        [OF legacy init flat_closed norm shared finite card_bound member_size cubic])
  show "rsizes (rpders_strong1_rows_raw (rerase (intern r)) s) \<le> B"
    by (rule
