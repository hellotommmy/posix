...
Write-Host "== Chapter 7 derivative-size compare: ks=$Ks lengths=$LengthList metrics=$Metrics seq=$SeqMode out=$CsvRel =="
...
if ($LASTEXITCODE -ne 0) {
  if ($LASTEXITCODE -eq 124) {
    throw "Chapter 7 derivative-size Scala run timed out after $TimeoutSeconds seconds"
  }
  throw "Chapter 7 derivative-size Scala run failed with exit code $LASTEXITCODE"
}
...
$PlotArgs = @(
  ...
  "--csv", $CsvAbs,
  "--out-dir", $OutDirPath,
  "--title-prefix", "Chapter 7 derivative-size comparison"
)
if ($LogY) {
  $PlotArgs += "--log-y"
}
...
python @PlotArgs
if ($LASTEXITCODE -ne 0) {
  throw "Chapter 7 derivative-size comparison plotting failed with exit code $LASTEXITCODE"
}

Write-Host "== Wrote Chapter 7 derivative-size comparison to $OutDirPath =="
