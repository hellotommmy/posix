...
| `MATHPROBLEM_ROWCOUNT.md` | LIVE | Self-contained statement of the D law (Antimirov row-count linearity) — the main remaining sub-problem of the cubic gate. Includes the falsified strengthenings with CEs and suggested attacks. Hand to a fresh single-focus session. |
| `GPT_PRO_DLAW_VERDICT.md` | LIVE | GPT Pro's design verdict on the D law (2026-06-13): the T+S telescoping boundary invariant that dissolves the SEQ overlap, the exact-E0 gap it found, four bridge lemmas, and per-constructor discharge. The recommended D-law architecture — sample-check at depth≥5 before Isabelle. |
