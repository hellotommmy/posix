...
| `GPT_PRO_GATE_BRIDGE_VERDICT4.md` | **LIVE — THE ROUTE** | GPT Pro verdict #5 (2026-06-14): the PROOF SKELETON for the #3/#4 cover lemmas (the additive acc-split was measured BOXED). A proof-only **`weak_child_drain`** carrier (non-collapsing `rsimp4` plug, S-FREE acc) + an **injective indexed-slot charge**, with the strong cover #5 proven by **mutual P/Q induction**. VALIDATED at depth≥5 (~660k checks, 0 in-fragment violations; W1 weak cover, W2/W3 RALTS, W4/W5 RSTAR; both CEs covered). Two caveats: weak acc S-FREE; guards `S p=p`. Implementation stack in §1-8; lanes in STEER.md. |
| `GPT_PRO_GATE_BRIDGE_VERDICT3.md` | LIVE (the BOUND) | GPT Pro verdict #4 (2026-06-14): the **context-cover ledger** `drain_ctxs` — bound the parent drain SIZE by linear context slots (declared cost survives the `a*·a*→a*` collapse), NOT set membership. The BOUND is validated + the infra/static lemmas are green; its #3/#4 cover PROOFS were deferred and are supplied by VERDICT4.md (weak carrier). |
| `GPT_PRO_GATE_BRIDGE_VERDICT2.md` | SUPERSEDED (§4) | GPT Pro verdict #3 (2026-06-14): the continuation-parametric drain potential over the CURRENT normalized rows (carrier strong_child_drain; measure rsize p). Its §5 ARITHMETIC landed green and is KEPT; its §4 per-child SET-containment step is CHECKED-FALSE for RALTS/RSTAR (guarded `a*·a*→a*` collapse) and is REPLACED by VERDICT3.md's context-cover. |
