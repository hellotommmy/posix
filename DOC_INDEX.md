...
| `GPT_PRO_GATE_BRIDGE_VERDICT3.md` | **LIVE — THE ROUTE** | GPT Pro verdict #4 (2026-06-14), the corrected design after the §4 per-child ALTS/STAR containment was checked-false: the **context-cover ledger** `drain_ctxs` — bound the parent drain SIZE by linear context slots (declared cost survives the `a*·a*→a*` collapse), NOT set membership. VALIDATED at depth≥5 (zero violations, ~580k cases; both old CEs covered). 6-lemma implementation stack in §5. Keeps `drain_pot`/`drain_w` + the green §5 arithmetic from verdict #3. |
