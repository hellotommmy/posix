...
lemma D1_RSEQ_le_rsize_from_combined_head_boundary_spine:
  fixes r1 r2 k
  assumes combined:
...
proof -
  ...
  have decomp: "?A = ?Root \<union> ?T1 \<union> ?T2"
    unfolding strong_apder_acc_single_RALTS_decomp single_term_def
      rsimpStrong_dlform_closure_def
    by auto
  have incl:
    "?A - ?B \<subseteq>
      ((?Root \<union> ?T1) - (?B \<union> ?Bc)) \<union>
      ((?Bc - ?B) \<union> (?T2 - ?B))"
    using decomp by auto
  have "D1 (RSEQ r1 r2) k \<le>
      card (((?Root \<union> ?T1) - (?B \<union> ?Bc)) \<union>
        ((?Bc - ?B) \<union> (?T2 - ?B)))"
    unfolding D1_def D_def
    by (rule card_mono) (use incl in auto)
  also have "... \<le>
      card ((?Root \<union> ?T1) - (?B \<union> ?Bc)) +
      card ((?Bc - ?B) \<union> (?T2 - ?B))"
    by (rule card_Un_le)
...
lemma D1_RALTS_le_rsize_from_L1_and_branches_spine:
  assumes L1_cover:
...
lemma singleton_bound_from_L1_BND_combined_RSEQ_clean_spine:
  assumes L1_cover:
  ...
  have combined:
    "card ((single_root (RSEQ r1 r2) k \<union>
        ...
        card ((strong_apder_acc RONE (rsimp4_SEQ_atom r2 k) -
          ...
          (single_term r2 k - strong_apder_acc RONE k))
        \<le> rsize (RSEQ r1 r2)"
    by (rule combined_RSEQ_clean) (use RSEQ.prems in auto)
  show ?case
    by (rule D1_RSEQ_le_rsize_from_combined_head_boundary_spine[OF combined])
next
  case (RALTS rs)
  have branch: "\<And>q. q \<in> set rs \<Longrightarrow> D1 q k \<le> rsize q"
    using RALTS.IH RALTS.prems by auto
  show ?case
    by (rule D1_RALTS_le_rsize_from_L1_and_branches_spine
        [OF L1_cover branch])
next
  case (RSTAR r)
  let ?c = "rsimp4_SEQ_atom (RSTAR r) k"
...
subsection \<open>Clean-domain final route from L1 and singleton D1\<close>

lemma card_strong_apder_acc_RALTS_diff_base_le_size_budget_from_D1_clean_spine:
  assumes L1_cover:
    "\<And>rs k. strong_apder_acc (RALTS rs) k \<subseteq>
      (\<Union>q \<in> set rs. strong_apder_acc (RALTS [q]) k)"
    and singleton_bound:
    "\<And>q k. legacy_rrexp q \<Longrightarrow> rntimes_free q \<Longrightarrow>
      apder_nf q \<Longrightarrow> apder_nf k \<Longrightarrow>
      D1 q k \<le> rsize q"
    and cleanrs: "apder_clean (RALTS rs)"
    and nfk: "apder_nf k"
  shows "D (RALTS rs) k \<le> ralts_size_budget rs"
proof (rule card_strong_apder_acc_RALTS_diff_base_le_size_budget_spine
    [OF L1_cover])
  fix q
  assume q: "q \<in> set rs"
  have cleanq: "apder_clean q"
    using cleanrs q by (simp add: apder_clean_def)
  have domain: "legacy_rrexp q \<and> rntimes_free q \<and> apder_nf q"
    by (rule apder_clean_domain_spine[OF cleanq])
  show "D (RALTS [q]) k \<le> rsize q"
    using singleton_bound[of q k] domain nfk
    by (simp add: D1_def)
qed

lemma card_strong_apder_acc_diff_base_le_rsize_clean_spine:
  assumes ralts_diff:
    "\<And>rs k. apder_clean (RALTS rs) \<Longrightarrow> apder_nf k \<Longrightarrow>
      D (RALTS rs) k \<le> rsize (RALTS rs)"
    and clean: "apder_clean r"
    and nfk: "apder_nf k"
  shows "D r k \<le> rsize r"
...
next
  case (RALTS rs)
  show ?case
    by (rule ralts_diff[OF RALTS.prems(1) RALTS.prems(2)])
next
  case (RSTAR r)
  let ?ks = "rsimp4_SEQ_atom (RSTAR r) k"
  let ?Root = "row_dlforms (rsimpStrong_raw ?ks)"
...
lemma card_strong_apder_acc_diff_base_le_rsize_from_L1_D1_clean_spine:
  assumes L1_cover:
    "\<And>rs k. strong_apder_acc (RALTS rs) k \<subseteq>
      (\<Union>q \<in> set rs. strong_apder_acc (RALTS [q]) k)"
    and singleton_bound:
    ...
    and nfk: "apder_nf k"
  shows "D r k \<le> rsize r"
proof (rule card_strong_apder_acc_diff_base_le_rsize_clean_spine
    [OF _ clean nfk])
  fix rs k
  assume cleanrs: "apder_clean (RALTS rs)"
    and nfk': "apder_nf k"
  have "D (RALTS rs) k \<le> ralts_size_budget rs"
    by (rule card_strong_apder_acc_RALTS_diff_base_le_size_budget_from_D1_clean_spine
        [OF L1_cover singleton_bound cleanrs nfk'])
  then show "D (RALTS rs) k \<le> rsize (RALTS rs)"
    by (simp add: ralts_size_budget_eq_rsizes)
qed

lemma card_apder_strong_dlfrontier_le_from_L1_D1_clean_spine:
  assumes L1_cover:
    "\<And>rs k. strong_apder_acc (RALTS rs) k \<subseteq>
      (\<Union>q \<in> set rs. strong_apder_acc (RALTS [q]) k)"
    and singleton_bound:
...
proof (rule card_apder_strong_dlfrontier_le_spine[OF _ clean])
  ...
  assume clean': "apder_clean r"
    and nfk: "apder_nf k"
  show "D r k \<le> rsize r"
    by (rule card_strong_apder_acc_diff_base_le_rsize_from_L1_D1_clean_spine
        [OF L1_cover singleton_bound clean' nfk])
qed

corollary cubic_gate_unconditional_from_L1_D1_clean_spine:
  assumes L1_cover:
    "\<And>rs k. strong_apder_acc (RALTS rs) k \<subseteq>
      (\<Union>q \<in> set rs. strong_apder_acc (RALTS [q]) k)"
    and singleton_bound:
...
text \<open>
Expected lane outputs for the final Route-1 adapter:
COVER: strong_apder_acc_RALTS_singleton_cover,
  strong_apder_acc_RALTS_nested_flatten.
BND: boundary_term_absorb.
SEQ: combined_RSEQ_clean_for_formalize.
This theory only consumes these interfaces; it does not prove the lane cruxes.
\<close>

theorem route1_cubic_final_from_lane_outputs:
  assumes L1_final:
    "\<And>rs k. strong_apder_acc (RALTS rs) k \<subseteq>
      (\<Union>q \<in> set rs. strong_apder_acc (RALTS [q]) k)"
    and L1_nested_flatten:
  ...
  shows "rsize_set
     ...
     \<le> 2 * (rsize r + 3) ^ 3"
  by (rule cubic_gate_unconditional_from_L1_BND_combined_RSEQ_clean_spine
      [OF L1_final L1_nested_flatten boundary_term_absorb
        combined_RSEQ_clean clean])
