...
lemma stupdates_join_general:
  shows  "concat (map hElem (map (rder x) (map (\<lambda>s1. RSEQ (rders r0 s1) (RSTAR r0)) 
            (star_updates xs r0 Ss))) ) =
...
datatype option1  = Some1 string nat | 
                    Some2 string

(*
fun fromUpdate ::  "char \<Rightarrow> rrexp \<Rightarrow>  option1  list \<Rightarrow> option1  list" where
  "nupdate c r [] = []"
| "nupdate c r (Some (s, Suc n) # Ss) = (if (rnullable (rders r s)) 
                                          then Some (s@[c], Suc n) # Some ([c], n) # (nupdate c r Ss) 
                                          else Some ((s@[c]), Suc n)  # (nupdate c r Ss) 
