...
lemma card_rpath9_atom_frontier_acc_RSTAR_rsimp9_RONE_productI:
  assumes body: "card (rpath9_atom_frontier_acc r
      (rsimp7_SEQ_atom (rsimp9 (RSTAR r))
        (rsimp7_SEQ_atom (rsimp9 k) RONE))) \<le>
      rsize r * (rsize (RSEQ (RSTAR r) k) + 2)"
  shows "card (rpath9_atom_frontier_acc (RSTAR r)
      (rsimp7_SEQ_atom (rsimp9 k) RONE)) \<le>
    rsize (RSTAR r) * (rsize (RSEQ (RSTAR r) k) + 2)"
proof (rule card_rpath9_atom_frontier_acc_RSTAR_productI)
  show "card (rpath9_atom_frontier_acc r
      (rsimp7_SEQ_atom (rsimp9 (RSTAR r))
        (rsimp7_SEQ_atom (rsimp9 k) RONE))) \<le>
      rsize r * (rsize (RSEQ (RSTAR r) k) + 2)"
    by (rule body)
...
lemma rpath9_atom_frontier_acc_RSTAR_rsimp9_RONE_member_sizeI:
  assumes body: "\<And>x. x \<in> rpath9_atom_frontier_acc r
      (rsimp7_SEQ_atom (rsimp9 (RSTAR r))
        (rsimp7_SEQ_atom (rsimp9 k) RONE)) \<Longrightarrow>
      rsize x \<le> rsize (RSEQ (RSTAR r) k)"
    and x: "x \<in> rpath9_atom_frontier_acc (RSTAR r)
      (rsimp7_SEQ_atom (rsimp9 k) RONE)"
  shows "rsize x \<le> rsize (RSEQ (RSTAR r) k)"
  by (rule rpath9_atom_frontier_acc_RSTAR_member_sizeI[OF body x])
...
lemma card_rpath9_atom_frontier_acc_RNTIMES_nonzero_rsimp9_RONE_productI:
  assumes m: "m \<noteq> 0"
    and body: "card (rpath9_atom_frontier_acc r
...
proof (rule card_rpath9_atom_frontier_acc_RNTIMES_nonzero_productI[OF m])
  show "card (rpath9_atom_frontier_acc r
      (rsimp7_SEQ_atom (rsimp9 (RNTIMES r (m - 1)))
        (rsimp7_SEQ_atom (rsimp9 k) RONE))) \<le>
      rsize r * (rsize (RSEQ (RNTIMES r (m - 1)) k) + 2)"
...
lemma rpath9_atom_frontier_acc_RNTIMES_nonzero_rsimp9_RONE_member_sizeI:
  assumes m: "m \<noteq> 0"
    and body: "\<And>x. x \<in> rpath9_atom_frontier_acc r
      (rsimp7_SEQ_atom (rsimp9 (RNTIMES r (m - 1)))
        (rsimp7_SEQ_atom (rsimp9 k) RONE)) \<Longrightarrow>
...
lemma rpath9_tail_prefix_continuation_bound_counterexample:
  "RSEQ (RCHAR b) (RSEQ (RCHAR c) (RCHAR d)) \<in>
    rpath9_atom_frontier_acc
      (RSEQ (RCHAR a) (RSEQ (RCHAR b) (RCHAR c)))
      (rpath9_tail (RSEQ (RCHAR d) RONE))"
  "rsize (RSEQ (RCHAR d) RONE) <
...
proof (rule rpath9_atom_frontiers_RSEQ_member_sizeI[OF _ right x])
  ...
  assume y: "y \<in> rpath9_atom_frontier_acc r1
      (rsimp7_SEQ_atom (rsimp9 r2) RONE)"
  have "y \<in> rpath9_atom_frontier_acc r1 (rpath9_tail (RSEQ r2 RONE))"
    using y by simp
  then show "rsize y \<le> Suc (rsize (RSEQ r1 r2) + rsize (RSEQ r1 r2))"
    by (rule left)
qed

lemma rpath9_atom_frontiers_RSTAR_member_size_rpath9_tail_parentI:
  assumes body: "\<And>x. x \<in> rpath9_atom_frontier_acc r
      (rpath9_tail (RSEQ (RSTAR r) RONE)) \<Longrightarrow>
      rsize x \<le> Suc (rsize (RSTAR r) + rsize (RSTAR r))"
    and x: "x \<in> rpath9_atom_frontiers (RSTAR r)"
...
proof (rule rpath9_atom_frontiers_RSTAR_member_sizeI[OF _ x])
  ...
  assume y: "y \<in> rpath9_atom_frontier_acc r
      (rsimp7_SEQ_atom (rsimp9 (RSTAR r)) RONE)"
  have "y \<in> rpath9_atom_frontier_acc r
      (rpath9_tail (RSEQ (RSTAR r) RONE))"
    using y by simp
  then show "rsize y \<le> Suc (rsize (RSTAR r) + rsize (RSTAR r))"
    by (rule body)
qed

lemma rpath9_atom_frontiers_RNTIMES_nonzero_member_size_rpath9_tail_parentI:
  assumes n: "n \<noteq> 0"
    and body: "\<And>x. x \<in> rpath9_atom_frontier_acc r
      (rpath9_tail (RSEQ (RNTIMES r (n - 1)) RONE)) \<Longrightarrow>
      rsize x \<le> Suc (rsize (RNTIMES r n) + rsize (RNTIMES r n))"
...
proof (rule rpath9_atom_frontiers_RNTIMES_nonzero_member_sizeI[OF n _ x])
  ...
  have "y \<in> rpath9_atom_frontier_acc r
      (rpath9_tail (RSEQ (RNTIMES r (n - 1)) RONE))"
    using y by simp
  then show "rsize y \<le> Suc (rsize (RNTIMES r n) + rsize (RNTIMES r n))"
    by (rule body)
qed
...
lemma rpath9_member_budget_nested_star_not_linear:
  "Suc (rsize (RSTAR (RSTAR (RCHAR a))) +
      rsize (RSTAR (RSTAR (RCHAR a)))) <
    rpath9_member_budget (RSTAR (RSTAR (RCHAR a))) RONE"
  by simp

fun rpath9_tight_member_budget :: "rrexp \<Rightarrow> rrexp \<Rightarrow> nat"
  and rpath9_tight_member_budget_list :: "rrexp list \<Rightarrow> rrexp \<Rightarrow> nat" where
  "rpath9_tight_member_budget RZERO k = 0"
| "rpath9_tight_member_budget RONE k = 0"
| "rpath9_tight_member_budget (RCHAR c) k = rsize (rpath9_tail k)"
| "rpath9_tight_member_budget (RALTS rs) k =
    rpath9_tight_member_budget_list rs k"
| "rpath9_tight_member_budget (RSEQ r1 r2) k =
    max (rpath9_tight_member_budget r1 (RSEQ r2 k))
      (rpath9_tight_member_budget r2 k)"
| "rpath9_tight_member_budget (RSTAR r) k =
    rpath9_tight_member_budget r (RSEQ (RSTAR r) k)"
| "rpath9_tight_member_budget (RNTIMES r n) k =
    (if n = 0 then 0 else
      rpath9_tight_member_budget r (RSEQ (RNTIMES r (n - 1)) k))"
| "rpath9_tight_member_budget (RBACKREF4 r1 r2 r3 r4 cs) k =
    max (rpath9_tight_member_budget r1 k)
      (max (rpath9_tight_member_budget r2 k)
        (max (rpath9_tight_member_budget r3 k)
          (rpath9_tight_member_budget r4 k)))"
| "rpath9_tight_member_budget (RHALF r cs rep) k =
    rpath9_tight_member_budget r k"
| "rpath9_tight_member_budget (RRESIDUE cs rep) k = 0"
| "rpath9_tight_member_budget_list [] k = 0"
| "rpath9_tight_member_budget_list (r # rs) k =
    max (rpath9_tight_member_budget r k)
      (rpath9_tight_member_budget_list rs k)"

lemma rpath9_tight_member_budget_list_member_le:
  assumes "r \<in> set rs"
...
next
  case (RCHAR c)
  have "rsize x \<le> rsize (rpath9_tail k)"
    using RCHAR.prems rfrontier_member_size_le_rsize by simp
  then show ?case by simp
next
  case (RALTS rs)
  then obtain r where r:
      ...
      "x \<in> rpath9_atom_frontier_acc r (rpath9_tail k)"
    by auto
  have "rsize x \<le> rpath9_tight_member_budget r k"
    using RALTS.hyps[OF r(1) r(2)] .
  also have "... \<le> rpath9_tight_member_budget_list rs k"
    by (rule rpath9_tight_member_budget_list_member_le[OF r(1)])
  finally show ?case
    by simp
next
  case (RSEQ r1 r2)
  ...
  proof cases
    case 1
    have "rsize x \<le> rpath9_tight_member_budget r1 (RSEQ r2 k)"
      by (rule RSEQ.hyps(1)[OF 1])
    then show ?thesis
      by simp
  next
    case 2
    have "rsize x \<le> rpath9_tight_member_budget r2 k"
      by (rule RSEQ.hyps(2)[OF 2])
    then show ?thesis
      by simp
  qed
next
  ...
  have x: "x \<in> rpath9_atom_frontier_acc r
      (rpath9_tail (RSEQ (RSTAR r) k))"
    using RSTAR.prems by simp
  have "rsize x \<le> rpath9_tight_member_budget r (RSEQ (RSTAR r) k)"
    by (rule RSTAR.hyps[OF x])
  then show ?case
    by simp
next
  ...
  next
    ...
    have x: "x \<in> rpath9_atom_frontier_acc r
        (rpath9_tail (RSEQ (RNTIMES r (n - 1)) k))"
      using RNTIMES.prems Suc by simp
    have "rsize x \<le> rpath9_tight_member_budget r
        (RSEQ (RNTIMES r (n - 1)) k)"
      by (rule RNTIMES.hyps[OF x])
    then show ?thesis
      using Suc by simp
...
next
  ...
  proof cases
    case 1
    then have "rsize x \<le> rpath9_tight_member_budget r1 k"
      by (rule RBACKREF4.hyps(1))
    then show ?thesis by simp
  next
    case 2
    then have "rsize x \<le> rpath9_tight_member_budget r2 k"
      by (rule RBACKREF4.hyps(2))
    then show ?thesis by simp
  next
    case 3
    then have "rsize x \<le> rpath9_tight_member_budget r3 k"
      by (rule RBACKREF4.hyps(3))
    then show ?thesis by simp
  next
    case 4
    then have "rsize x \<le> rpath9_tight_member_budget r4 k"
      by (rule RBACKREF4.hyps(4))
    then show ?thesis by simp
  qed
next
...
lemma rpath9_atom_frontiers_tight_member_budget:
  assumes "x \<in> rpath9_atom_frontiers r"
  shows "rsize x \<le> rpath9_tight_member_budget r RONE"
proof -
  have "x \<in> rpath9_atom_frontier_acc r (rpath9_tail RONE)"
    using assms by (simp add: rpath9_atom_frontiers_def)
  then show ?thesis
    by (rule rpath9_atom_frontier_acc_rpath9_tail_tight_member_budget)
qed

lemma rpath9_tight_member_budget_nested_star_linear_sanity:
...
lemma rsimp7_SEQ_atom_RONE_eq_rsimp4:
...
proof
  fix x
  assume x: "x \<in> set (rflts [r])"
  show "x \<in> rfrontier r"
  proof (cases r)
    case RZERO
    then show ?thesis
      using x by simp
  next
    ...
    have good_alt: "good (RALTS rs)"
      using assms RALTS by simp
    have x_rs: "x \<in> set rs"
      using x RALTS by simp
    have x_good: "good x \<and> nonalt x"
      using good_alt x_rs good_RALTS_elem by blast
    have "x \<noteq> RZERO"
      using x_good good_not_RZERO by blast
    then have "x \<in> rfrontier x"
      by (rule rfrontier_nonzero_nonalt_self) (use x_good in simp)
    then have "x \<in> rfrontiers rs"
      using x_rs rfrontiers_member_iff by blast
    then show ?thesis
      using RALTS by simp
...
lemma rder_path_continuations_acc_RCHAR_root_path9_stable:
  assumes norm_tail: "rsimp9 (rsimp4_SEQ_atom root RONE) = rsimp9 root"
    and p: "p \<in> rder_path_continuations_acc c (RCHAR d)
      (rsimp4_SEQ_atom root RONE)"
  shows "set (rflts [rsimp9 p]) \<subseteq>
    partial_derivative_path9_atom_frontier_universe root"
proof (cases "c = d")
  case True
  have p_eq: "p = rsimp4_SEQ_atom root RONE"
    using p True by simp
  have "set (rflts [rsimp9 p]) \<subseteq>
      set (rflts [rsimp9 root])"
    using p_eq norm_tail by simp
  then show ?thesis
    using rflts_singleton_rsimp9_path9_atom_frontier[of root] by blast
next
  case False
  then show ?thesis
    using p by simp
...
lemma rder_path_continuations_acc_RCHAR_root_path9_RSTAR:
  ...
  by (rule rder_path_continuations_acc_RCHAR_root_path9_stable)
    (use assms in simp_all)

lemma rder_path_continuations_acc_RCHAR_root_path9_RNTIMES:
  assumes "p \<in> rder_path_continuations_acc c (RCHAR d)
      (rsimp4_SEQ_atom (RNTIMES r n) RONE)"
  shows "set (rflts [rsimp9 p]) \<subseteq>
...
proof (cases "c = d")
  case True
  have p_eq: "p = rsimp4_SEQ_atom r2 RONE"
    using p True by simp
  show ?thesis
  proof
    fix x
    assume x: "x \<in> set (rflts [rsimp9 p])"
    have x_flat: "x \<in> set (rflts [rsimp9 r2])"
      using x p_eq norm_tail by simp
    have good_norm: "good (rsimp9 r2) \<or> rsimp9 r2 = RZERO"
      by (rule good_rsimp9)
    have "x \<in> rfrontier (rsimp9 r2)"
      using rflts_good_singleton_frontier[OF good_norm] x_flat by blast
    then have "x \<in> rfrontier (rsimp7_SEQ_atom (rsimp9 r2) RONE)"
      using stable by (simp add: rsimp7_SEQ_atom_RONE_eq_rsimp4)
    then have "x \<in> rpath9_atom_frontiers (RSEQ (RCHAR d) r2)"
      by (simp add: rpath9_atom_frontiers_def)
    then show "x \<in> partial_derivative_path9_atom_frontier_universe
        (RSEQ (RCHAR d) r2)"
      by (simp add: partial_derivative_path9_atom_frontier_universe_def)
...
lemma rder_path_continuations_acc_RCHAR_left_path9_RZERO:
  assumes "p \<in> rder_path_continuations_acc c (RCHAR d)
      (rsimp4_SEQ_atom RZERO RONE)"
  shows "set (rflts [rsimp9 p]) \<subseteq>
    partial_derivative_path9_atom_frontier_universe (RSEQ (RCHAR d) RZERO)"
  by (rule rder_path_continuations_acc_RCHAR_left_path9_stable)
    (use assms in simp_all)

lemma rder_path_continuations_acc_RCHAR_left_path9_RONE:
  assumes "p \<in> rder_path_continuations_acc c (RCHAR d)
      (rsimp4_SEQ_atom RONE RONE)"
  shows "set (rflts [rsimp9 p]) \<subseteq>
    partial_derivative_path9_atom_frontier_universe (RSEQ (RCHAR d) RONE)"
  by (rule rder_path_continuations_acc_RCHAR_left_path9_stable)
    (use assms in simp_all)

lemma rder_path_continuations_acc_RCHAR_left_path9_RCHAR:
  assumes "p \<in> rder_path_continuations_acc c (RCHAR d)
      (rsimp4_SEQ_atom (RCHAR e) RONE)"
  shows "set (rflts [rsimp9 p]) \<subseteq>
    partial_derivative_path9_atom_frontier_universe (RSEQ (RCHAR d) (RCHAR e))"
  by (rule rder_path_continuations_acc_RCHAR_left_path9_stable)
    (use assms in simp_all)

lemma rder_path_continuations_acc_RCHAR_left_path9_RSTAR:
  assumes "p \<in> rder_path_continuations_acc c (RCHAR d)
      (rsimp4_SEQ_atom (RSTAR r) RONE)"
  shows "set (rflts [rsimp9 p]) \<subseteq>
    partial_derivative_path9_atom_frontier_universe (RSEQ (RCHAR d) (RSTAR r))"
  by (rule rder_path_continuations_acc_RCHAR_left_path9_stable)
    (use assms in \<open>simp_all split: rrexp.splits\<close>)

lemma rder_path_continuations_acc_RCHAR_left_path9_RNTIMES:
  assumes "p \<in> rder_path_continuations_acc c (RCHAR d)
      (rsimp4_SEQ_atom (RNTIMES r n) RONE)"
  shows "set (rflts [rsimp9 p]) \<subseteq>
    partial_derivative_path9_atom_frontier_universe (RSEQ (RCHAR d) (RNTIMES r n))"
  by (rule rder_path_continuations_acc_RCHAR_left_path9_stable)
    (use assms in \<open>simp_all split: rrexp.splits\<close>)
...
lemma rpath9_atom_frontiers_seq_alt_left_subset:
  ...
  shows "rpath9_atom_frontier_acc q
      (rsimp7_SEQ_atom (rsimp9 r2) RONE) \<subseteq>
    rpath9_atom_frontiers (RSEQ (RALTS rs) r2)"
  using assms by (auto simp add: rpath9_atom_frontiers_def)

lemma rpath9_atom_frontiers_seq_alt_left_universe:
  assumes q: "q \<in> set rs"
    and x: "x \<in> rpath9_atom_frontier_acc q
      (rsimp7_SEQ_atom (rsimp9 r2) RONE)"
  shows "x \<in> partial_derivative_path9_atom_frontier_universe
    (RSEQ (RALTS rs) r2)"
  using rpath9_atom_frontiers_seq_alt_left_subset[OF q] x
    rpath9_atom_frontiers_universe[of "RSEQ (RALTS rs) r2"]
  by blast

lemma rder_path_continuations_acc_RCHAR_alt_left_path9_stable:
...
proof (cases "c = d")
  ...
  proof
    ...
    have good_norm: "good (rsimp9 r2) \<or> rsimp9 r2 = RZERO"
      by (rule good_rsimp9)
    have x_front: "x \<in> rfrontier (rsimp9 r2)"
      using rflts_good_singleton_frontier[OF good_norm] x_flat by blast
    have "x \<in> rfrontier (rsimp7_SEQ_atom (rsimp9 r2) RONE)"
      using x_front stable by (simp add: rsimp7_SEQ_atom_RONE_eq_rsimp4)
    then have x_acc: "x \<in> rpath9_atom_frontier_acc (RCHAR d)
        (rsimp7_SEQ_atom (rsimp9 r2) RONE)"
      by simp
    show "x \<in> partial_derivative_path9_atom_frontier_universe
        (RSEQ (RALTS rs) r2)"
      by (rule rpath9_atom_frontiers_seq_alt_left_universe[OF q x_acc])
  qed
next
  case False
  then show ?thesis
...
lemma rder_path_continuations_acc_RALTS_RCHARs_left_path9_stable:
  assumes chars: "\<And>q. q \<in> set rs \<Longrightarrow> \<exists>d. q = RCHAR d"
    and norm_tail:
      "rsimp9 (rsimp4_SEQ_atom r2 RONE) = rsimp9 r2"
    and stable:
...
proof (rule rder_path_continuations_acc_RALTS_carriedI[OF _ p])
  fix q p
  assume q: "q \<in> set rs"
    and p: "p \<in> rder_path_continuations_acc c q
      (rsimp4_SEQ_atom r2 RONE)"
  obtain d where q_eq: "q = RCHAR d"
    using chars[OF q] by blast
  have q_char: "RCHAR d \<in> set rs"
    using q q_eq by simp
  have p_char: "p \<in> rder_path_continuations_acc c (RCHAR d)
      (rsimp4_SEQ_atom r2 RONE)"
    using p q_eq by simp
  show "set (rflts [rsimp9 p]) \<subseteq>
      partial_derivative_path9_atom_frontier_universe (RSEQ (RALTS rs) r2)"
...
lemma rpder_norm9_path9_atom_frontier_step_RSEQ_RCHAR_stable:
  assumes norm_tail:
    ...
    and stable:
      "rsimp4_SEQ_atom (rsimp9 r2) RONE = rsimp9 r2"
  shows "set (rflts (rpder_norm9_list c (RSEQ (RCHAR d) r2))) \<subseteq>
    partial_derivative_path9_atom_frontier_universe (RSEQ (RCHAR d) r2)"
proof (rule rpder_norm9_path9_atom_frontier_step_RSEQ_directI)
  show "legacy_rrexp (RCHAR d)"
    by simp
next
  fix p
  assume p: "p \<in> rder_path_continuations_acc c (RCHAR d)
      (rsimp4_SEQ_atom r2 RONE)"
  show "set (rflts [rsimp9 p]) \<subseteq>
      partial_derivative_path9_atom_frontier_universe (RSEQ (RCHAR d) r2)"
    by (rule rder_path_continuations_acc_RCHAR_left_path9_stable
        [OF norm_tail stable p])
next
  assume "rnullable (RCHAR d)"
  then show "set (rflts (rpder_norm9_list c r2)) \<subseteq>
      partial_derivative_path9_atom_frontier_universe (RSEQ (RCHAR d) r2)"
    by simp
qed

lemma rpder_norm9_path9_atom_frontier_step_RSEQ_RCHAR_RZERO:
  "set (rflts (rpder_norm9_list c (RSEQ (RCHAR d) RZERO))) \<subseteq>
    partial_derivative_path9_atom_frontier_universe (RSEQ (RCHAR d) RZERO)"
  by (rule rpder_norm9_path9_atom_frontier_step_RSEQ_RCHAR_stable)
    simp_all

lemma rpder_norm9_path9_atom_frontier_step_RSEQ_RCHAR_RONE:
  "set (rflts (rpder_norm9_list c (RSEQ (RCHAR d) RONE))) \<subseteq>
    partial_derivative_path9_atom_frontier_universe (RSEQ (RCHAR d) RONE)"
  by (rule rpder_norm9_path9_atom_frontier_step_RSEQ_RCHAR_stable)
    simp_all

lemma rpder_norm9_path9_atom_frontier_step_RSEQ_RCHAR_RCHAR:
  "set (rflts (rpder_norm9_list c (RSEQ (RCHAR d) (RCHAR e)))) \<subseteq>
    partial_derivative_path9_atom_frontier_universe (RSEQ (RCHAR d) (RCHAR e))"
  by (rule rpder_norm9_path9_atom_frontier_step_RSEQ_RCHAR_stable)
    simp_all

lemma rpder_norm9_path9_atom_frontier_step_RSEQ_RCHAR_RSTAR:
  "set (rflts (rpder_norm9_list c (RSEQ (RCHAR d) (RSTAR r)))) \<subseteq>
    partial_derivative_path9_atom_frontier_universe (RSEQ (RCHAR d) (RSTAR r))"
  by (rule rpder_norm9_path9_atom_frontier_step_RSEQ_RCHAR_stable)
    (simp_all split: rrexp.splits)

lemma rpder_norm9_path9_atom_frontier_step_RSEQ_RCHAR_RNTIMES:
  "set (rflts (rpder_norm9_list c (RSEQ (RCHAR d) (RNTIMES r n)))) \<subseteq>
    partial_derivative_path9_atom_frontier_universe
      (RSEQ (RCHAR d) (RNTIMES r n))"
  by (rule rpder_norm9_path9_atom_frontier_step_RSEQ_RCHAR_stable)
    (simp_all split: rrexp.splits)
...
lemma rpder_norm9_path9_atom_frontier_step_RSEQ_RALTS_RCHARs_stable:
  assumes chars: "\<And>q. q \<in> set rs \<Longrightarrow> \<exists>d. q = RCHAR d"
    ...
    and stable:
      "rsimp4_SEQ_atom (rsimp9 r2) RONE = rsimp9 r2"
  shows "set (rflts (rpder_norm9_list c (RSEQ (RALTS rs) r2))) \<subseteq>
    partial_derivative_path9_atom_frontier_universe (RSEQ (RALTS rs) r2)"
proof (rule rpder_norm9_path9_atom_frontier_step_RSEQ_directI)
  show "legacy_rrexp (RALTS rs)"
  proof -
    have "\<forall>q \<in> set rs. legacy_rrexp q"
    proof
      fix q
      assume q: "q \<in> set rs"
      then obtain d where "q = RCHAR d"
        using chars by blast
      then show "legacy_rrexp q"
        by simp
    qed
    then show ?thesis
      by simp
...
next
  fix p
  assume p: "p \<in> rder_path_continuations_acc c (RALTS rs)
      (rsimp4_SEQ_atom r2 RONE)"
  show "set (rflts [rsimp9 p]) \<subseteq>
      partial_derivative_path9_atom_frontier_universe (RSEQ (RALTS rs) r2)"
    by (rule rder_path_continuations_acc_RALTS_RCHARs_left_path9_stable
        [OF chars norm_tail stable p])
next
  assume nullable: "rnullable (RALTS rs)"
  have False
  proof -
    obtain q where q: "q \<in> set rs" and nullable_q: "rnullable q"
      using nullable by auto
    obtain d where q_eq: "q = RCHAR d"
      using chars[OF q] by blast
    show ?thesis
      using nullable_q q_eq by simp
  qed
  then show "set (rflts (rpder_norm9_list c r2)) \<subseteq>
      partial_derivative_path9_atom_frontier_universe (RSEQ (RALTS rs) r2)"
    by simp
...
lemma rpder_norm9_path9_atom_frontier_step_RSEQ_RALTS_RCHARs_RZERO:
  assumes chars: "\<And>q. q \<in> set rs \<Longrightarrow> \<exists>d. q = RCHAR d"
  shows "set (rflts (rpder_norm9_list c (RSEQ (RALTS rs) RZERO))) \<subseteq>
    partial_derivative_path9_atom_frontier_universe (RSEQ (RALTS rs) RZERO)"
  by (rule rpder_norm9_path9_atom_frontier_step_RSEQ_RALTS_RCHARs_stable)
    (use chars in simp_all)

lemma rpder_norm9_path9_atom_frontier_step_RSEQ_RALTS_RCHARs_RONE:
  assumes chars: "\<And>q. q \<in> set rs \<Longrightarrow> \<exists>d. q = RCHAR d"
  shows "set (rflts (rpder_norm9_list c (RSEQ (RALTS rs) RONE))) \<subseteq>
    partial_derivative_path9_atom_frontier_universe (RSEQ (RALTS rs) RONE)"
  by (rule rpder_norm9_path9_atom_frontier_step_RSEQ_RALTS_RCHARs_stable)
    (use chars in simp_all)

lemma rpder_norm9_path9_atom_frontier_step_RSEQ_RALTS_RCHARs_RCHAR:
  assumes chars: "\<And>q. q \<in> set rs \<Longrightarrow> \<exists>d. q = RCHAR d"
  shows "set (rflts (rpder_norm9_list c (RSEQ (RALTS rs) (RCHAR d)))) \<subseteq>
    partial_derivative_path9_atom_frontier_universe (RSEQ (RALTS rs) (RCHAR d))"
  by (rule rpder_norm9_path9_atom_frontier_step_RSEQ_RALTS_RCHARs_stable)
    (use chars in simp_all)

lemma rpder_norm9_path9_atom_frontier_step_RSEQ_RALTS_RCHARs_RSTAR:
  assumes chars: "\<And>q. q \<in> set rs \<Longrightarrow> \<exists>d. q = RCHAR d"
  shows "set (rflts (rpder_norm9_list c (RSEQ (RALTS rs) (RSTAR r)))) \<subseteq>
    partial_derivative_path9_atom_frontier_universe (RSEQ (RALTS rs) (RSTAR r))"
  by (rule rpder_norm9_path9_atom_frontier_step_RSEQ_RALTS_RCHARs_stable)
    (use chars in \<open>simp_all split: rrexp.splits\<close>)

lemma rpder_norm9_path9_atom_frontier_step_RSEQ_RALTS_RCHARs_RNTIMES:
  assumes chars: "\<And>q. q \<in> set rs \<Longrightarrow> \<exists>d. q = RCHAR d"
  shows "set (rflts (rpder_norm9_list c
      (RSEQ (RALTS rs) (RNTIMES r n)))) \<subseteq>
    partial_derivative_path9_atom_frontier_universe
...
next
  fix p
  assume p: "p \<in> rder_path_continuations_acc c (RCHAR d)
      (rsimp4_SEQ_atom (RSTAR (RCHAR d)) RONE)"
  show "set (rflts [rsimp9 p]) \<subseteq>
      partial_derivative_path9_atom_frontier_universe (RSTAR (RCHAR d))"
    by (rule rder_path_continuations_acc_RCHAR_root_path9_RSTAR[OF p])
qed

lemma rder_path_continuations_acc_RALTS_RCHARs_root_path9_RSTAR:
  assumes chars: "\<And>q. q \<in> set rs \<Longrightarrow> \<exists>d. q = RCHAR d"
    and p: "p \<in> rder_path_continuations_acc c (RALTS rs)
      (rsimp4_SEQ_atom (RSTAR (RALTS rs)) RONE)"
  shows "set (rflts [rsimp9 p]) \<subseteq>
...
proof (rule rder_path_continuations_acc_RALTS_carriedI[OF _ p])
  ...
  show "set (rflts [rsimp9 p]) \<subseteq>
      partial_derivative_path9_atom_frontier_universe (RSTAR (RALTS rs))"
    by (rule rder_path_continuations_acc_RCHAR_root_path9_RSTAR[OF p_char])
qed

lemma rpder_norm9_path9_atom_frontier_step_RSTAR_RALTS_RCHARs:
  assumes chars: "\<And>q. q \<in> set rs \<Longrightarrow> \<exists>d. q = RCHAR d"
  shows "set (rflts (rpder_norm9_list c (RSTAR (RALTS rs)))) \<subseteq>
    partial_derivative_path9_atom_frontier_universe (RSTAR (RALTS rs))"
proof (rule rpder_norm9_path9_atom_frontier_step_RSTAR_directI)
  show "legacy_rrexp (RALTS rs)"
  proof -
...
lemma rpder_norm9_path9_atom_frontier_step_RNTIMES_RCHAR:
  "set (rflts (rpder_norm9_list c (RNTIMES (RCHAR d) n))) \<subseteq>
...
next
  ...
  assume n: "n \<noteq> 0"
    and p: "p \<in> rder_path_continuations_acc c (RCHAR d)
      (rsimp4_SEQ_atom (RNTIMES (RCHAR d) (n - 1)) RONE)"
  show "set (rflts [rsimp9 p]) \<subseteq>
      partial_derivative_path9_atom_frontier_universe (RNTIMES (RCHAR d) n)"
  proof (cases n)
    case 0
  ...
  next
    ...
    proof (cases "c = d")
      case True
      have p_eq: "p = RNTIMES (RCHAR d) m"
        using p Suc True by simp
      show ?thesis
      proof
        fix x
        assume x: "x \<in> set (rflts [rsimp9 p])"
        have x_front: "x \<in> rfrontier (rsimp9 (RNTIMES (RCHAR d) m))"
          using x p_eq rflts_singleton_rsimp9_frontier[of "RNTIMES (RCHAR d) m"]
          by blast
        have tail_eq:
          "rsimp7_SEQ_atom (rsimp9 (RNTIMES (RCHAR d) m)) RONE =
            rsimp9 (RNTIMES (RCHAR d) m)"
          by (cases m) (simp_all add: rsimp7_SEQ_atom_def)
        then have x_acc: "x \<in> rpath9_atom_frontier_acc (RCHAR d)
            (rsimp7_SEQ_atom (rsimp9 (RNTIMES (RCHAR d) m)) RONE)"
          using x_front by simp
        have x_acc_n: "x \<in> rpath9_atom_frontier_acc (RCHAR d)
            (rsimp7_SEQ_atom
              (rsimp9 (RNTIMES (RCHAR d) (n - 1))) RONE)"
          using x_acc Suc by simp
        show "x \<in> partial_derivative_path9_atom_frontier_universe
            (RNTIMES (RCHAR d) n)"
          by (rule rpath9_atom_frontiers_ntimes_body_universe[OF n x_acc_n])
      qed
    next
      case False
      then show ?thesis
...
lemma rder_path_continuations_acc_RALTS_RCHARs_root_path9_RNTIMES:
  assumes chars: "\<And>q. q \<in> set rs \<Longrightarrow> \<exists>d. q = RCHAR d"
    and n: "n \<noteq> 0"
    and p: "p \<in> rder_path_continuations_acc c (RALTS rs)
      (rsimp4_SEQ_atom (RNTIMES (RALTS rs) (n - 1)) RONE)"
  shows "set (rflts [rsimp9 p]) \<subseteq>
...
next
  ...
  proof (rule rder_path_continuations_acc_RALTS_carriedI[OF _ p])
    ...
    proof (cases "c = d")
      case True
      have p_eq: "p = RNTIMES (RALTS rs) m"
        using p_char n_eq True by simp
      show ?thesis
      proof (cases m)
        case 0
        show ?thesis
      ...
      next
        ...
        proof
          fix x
          assume x: "x \<in> set (rflts [rsimp9 p])"
          have x_front: "x \<in> rfrontier (rsimp9 (RNTIMES (RALTS rs) m))"
            using x p_eq rflts_singleton_rsimp9_frontier[of "RNTIMES (RALTS rs) m"]
            by blast
          have tail_eq:
            "rsimp7_SEQ_atom (rsimp9 (RNTIMES (RALTS rs) m)) RONE =
              rsimp9 (RNTIMES (RALTS rs) m)"
            using m_eq
            by (cases "rsimp9 (RALTS rs)")
              (simp_all add: rsimp7_SEQ_atom_def)
          then have x_acc_child: "x \<in> rpath9_atom_frontier_acc (RCHAR d)
              (rsimp7_SEQ_atom (rsimp9 (RNTIMES (RALTS rs) m)) RONE)"
            using x_front by simp
          have x_acc_alt: "x \<in> rpath9_atom_frontier_acc (RALTS rs)
              (rsimp7_SEQ_atom
                (rsimp9 (RNTIMES (RALTS rs) (n - 1))) RONE)"
            using q_char x_acc_child n_eq by force
          show "x \<in> partial_derivative_path9_atom_frontier_universe
              (RNTIMES (RALTS rs) n)"
            by (rule rpath9_atom_frontiers_ntimes_body_universe[OF n x_acc_alt])
        qed
      qed
    next
      case False
      then show ?thesis
        using p_char n_eq by simp
    qed
  qed
qed

lemma rpder_norm9_path9_atom_frontier_step_RNTIMES_RALTS_RCHARs:
  assumes chars: "\<And>q. q \<in> set rs \<Longrightarrow> \<exists>d. q = RCHAR d"
  shows "set (rflts (rpder_norm9_list c (RNTIMES (RALTS rs) n))) \<subseteq>
    partial_derivative_path9_atom_frontier_universe (RNTIMES (RALTS rs) n)"
proof (rule rpder_norm9_path9_atom_frontier_step_RNTIMES_directI)
  show "legacy_rrexp (RALTS rs)"
  proof -
...
lemma row_group_nf_rsimpStrong_prune_pair:
  assumes earlier_nf: "row_group_nf earlier"
    and later_nf: "row_group_nf later"
  shows "row_group_nf (rsimpStrong_prune_pair earlier later)"
proof -
  consider
    (shared) lrs rrs k where
      "earlier = RSEQ (RALTS lrs) k"
  ...
  proof cases
    case (shared lrs rrs k)
    have rrs_nf: "\<forall>r \<in> set rrs. row_group_nf r"
      using later_nf shared by simp
    have k_nf: "row_group_nf k"
      using later_nf shared by simp
    show ?thesis
      using shared
      by (simp add: rsimpStrong_prune_pair_def
          row_group_nf_shared_prune_result[OF rrs_nf k_nf])
  next
    case other
    have "rsimpStrong_prune_pair earlier later = later"
      using other
      ...
      by (cases earlier; cases later) (auto split: rrexp.splits)
    then show ?thesis
      using later_nf by simp
  qed
qed

lemma row_group_nf_rsimpStrong_prune_against_rows:
  assumes seen: "\<forall>x \<in> set seen. row_group_nf x"
    and row: "row_group_nf r"
  shows "row_group_nf (rsimpStrong_prune_against_rows seen r)"
  using seen row
...
next
  ...
  have x_nf: "row_group_nf x"
    using Cons.prems by simp
  have xs_nf: "\<forall>y \<in> set xs. row_group_nf y"
    using Cons.prems by simp
  have p_nf: "row_group_nf ?p"
    by (rule row_group_nf_rsimpStrong_prune_pair[OF x_nf Cons.prems(2)])
  have rest: "row_group_nf (rsimpStrong_prune_against_rows xs ?p)"
    by (rule Cons.hyps[OF xs_nf p_nf])
  show ?case
    using rest by simp
qed

lemma row_group_nf_rsimpStrong_prune_rows_acc:
  assumes seen: "\<forall>x \<in> set seen. row_group_nf x"
    and rows: "\<forall>x \<in> set rs. row_group_nf x"
  shows "\<forall>x \<in> set (rsimpStrong_prune_rows_acc seen rs). row_group_nf x"
  using rows seen
proof (induct rs arbitrary: seen)
  case Nil
  then show ?case by simp
next
  case (Cons r rs)
  let ?r' = "rsimpStrong_prune_against_rows seen r"
  have r_nf: "row_group_nf r"
    using Cons.prems by simp
  have rs_nf: "\<forall>x \<in> set rs. row_group_nf x"
    using Cons.prems by simp
  have r'_nf: "row_group_nf ?r'"
    by (rule row_group_nf_rsimpStrong_prune_against_rows
        [OF Cons.prems(2) r_nf])
  have seen': "\<forall>x \<in> set (?r' # seen). row_group_nf x"
    using r'_nf Cons.prems(2) by simp
  have tail: "\<forall>x \<in> set (rsimpStrong_prune_rows_acc (?r' # seen) rs).
      row_group_nf x"
    by (rule Cons.hyps[OF rs_nf seen'])
  show ?case
    using r'_nf tail by (simp add: Let_def)
qed

lemma row_group_nf_rsimpStrong_prune_rows:
  assumes "\<forall>x \<in> set rs. row_group_nf x"
  shows "\<forall>x \<in> set (rsimpStrong_prune_rows rs). row_group_nf x"
  unfolding rsimpStrong_prune_rows_def
  by (rule row_group_nf_rsimpStrong_prune_rows_acc)
    (use assms in simp_all)

lemma row_group_nf_rsimpStrong_ALTs:
  assumes "\<forall>x \<in> set rs. row_group_nf x"
  shows "row_group_nf (rsimpStrong_ALTs rs)"
proof -
  have pruned: "\<forall>x \<in> set (rsimpStrong_prune_rows rs). row_group_nf x"
    by (rule row_group_nf_rsimpStrong_prune_rows[OF assms])
  show ?thesis
    unfolding rsimpStrong_ALTs_def
    by (rule row_group_nf_normalize[OF pruned])
qed
...
lemma rsimpStrong_prune_pair_later_shared_subsetI:
  assumes later: "set (rflts [later]) \<subseteq> U"
    and shared: "\<And>lrs rrs k.
...
lemma rsimpStrong_prune_rows_later_shared_subsetI:
  assumes rows: "\<And>r. r \<in> set rs \<Longrightarrow> set (rflts [r]) \<subseteq> U"
    and shared: "\<And>lrs rrs k.
      RSEQ (RALTS rrs) k \<in> U \<Longrightarrow>
      set (rflts [rsimp7_SEQ_atom
...
proof (rule rsimpStrong_prune_rows_pair_subsetI[OF rows])
  ...
  assume later: "set (rflts [later]) \<subseteq> U"
  show "set (rflts [rsimpStrong_prune_pair earlier later]) \<subseteq> U"
    by (rule rsimpStrong_prune_pair_later_shared_subsetI[OF later shared])
qed
...
lemma row_group_nf_rsimpStrong:
  assumes "row_group_nf r"
  shows "row_group_nf (rsimpStrong r)"
  using assms
proof (induct r)
  case RZERO
  then show ?case by simp
...
next
  case (RSEQ r1 r2)
  have r1_nf: "row_group_nf r1"
    using RSEQ.prems by simp
  have r2_nf: "row_group_nf r2"
    using RSEQ.prems by simp
  have left: "row_group_nf (rsimpStrong r1)"
    by (rule RSEQ.hyps(1)[OF r1_nf])
  have right: "row_group_nf (rsimpStrong r2)"
    by (rule RSEQ.hyps(2)[OF r2_nf])
  show ?case
    by (simp add: row_group_nf_rsimp7_SEQ_atom[OF left right])
next
  case (RALTS rs)
  have elems: "\<forall>r \<in> set rs. row_group_nf (rsimpStrong r)"
  proof
    fix r
    assume r: "r \<in> set rs"
    have "row_group_nf r"
      using RALTS.prems r by simp
    then show "row_group_nf (rsimpStrong r)"
      by (rule RALTS.hyps[OF r])
  qed
  have flat: "\<forall>x \<in> set (rflts (map rsimpStrong rs)). row_group_nf x"
    by (rule row_group_nf_rflts) (use elems in auto)
  show ?case
    by (simp add: row_group_nf_rsimpStrong_ALTs[OF flat])
next
  case (RSTAR r)
  then show ?case
    by (cases "rsimpStrong r") simp_all
next
  case (RNTIMES r n)
  then show ?case by simp
next
...
definition rsimpStrong_prune_pair_raw :: "rrexp \<Rightarrow> rrexp \<Rightarrow> rrexp" where
...
lemma RL_rsimpStrong_prune_pair_raw_shared_suffix:
  "RL (RALTS [RSEQ (RALTS lrs) k,
      rsimpStrong_prune_pair_raw (RSEQ (RALTS lrs) k) (RSEQ (RALTS rrs) k)]) =
    RL (RALTS [RSEQ (RALTS lrs) k, RSEQ (RALTS rrs) k])"
proof -
  let ?pruned = "rprune_eq_against lrs rrs"
  have "RL (RALTS [RSEQ (RALTS lrs) k,
      rsimpStrong_prune_pair_raw (RSEQ (RALTS lrs) k) (RSEQ (RALTS rrs) k)]) =
    (RL (RALTS lrs) ;; RL k) \<union> (RL (rsimp_ALTs ?pruned) ;; RL k)"
    by (simp add: rsimpStrong_prune_pair_raw_def RL_rsimp7_SEQ_atom)
  also have "... =
    (RL (RALTS lrs) ;; RL k) \<union>
      ((\<Union> (set (map RL ?pruned))) ;; RL k)"
    by (simp add: RL_rsimp_RALTS)
  also have "... =
    (RL (RALTS lrs) ;; RL k) \<union>
      ((\<Union>r \<in> set ?pruned. RL r) ;; RL k)"
    by simp
...
lemma RL_rsimpStrong_prune_pair_raw_with_earlier:
  "RL earlier \<union> RL (rsimpStrong_prune_pair_raw earlier later) =
    RL earlier \<union> RL later"
proof -
  consider
    (shared) lrs rrs k where
  ...
  proof cases
    case (shared lrs rrs k)
    show ?thesis
      using RL_rsimpStrong_prune_pair_raw_shared_suffix[of lrs k rrs] shared
      by simp
  next
    case other
...
fun rsimpStrong_prune_against_rows_raw :: "rrexp list \<Rightarrow> rrexp \<Rightarrow> rrexp" where
...
fun rsimpStrong_prune_rows_acc_raw :: "rrexp list \<Rightarrow> rrexp list \<Rightarrow> rrexp list" where
...
lemma RL_rsimpStrong_prune_against_rows_raw:
  "RL (RALTS (seen @ [rsimpStrong_prune_against_rows_raw seen r])) =
    RL (RALTS (seen @ [r]))"
proof (induct seen arbitrary: r)
  case Nil
  then show ?case
...
next
  ...
  have tail:
    "RL (RALTS (xs @ [rsimpStrong_prune_against_rows_raw xs ?p])) =
      RL (RALTS (xs @ [?p]))"
    by (rule Cons.hyps)
  have pair: "RL x \<union> RL ?p = RL x \<union> RL r"
    by (rule RL_rsimpStrong_prune_pair_raw_with_earlier)
  show ?case
    using tail pair by auto
qed

lemma RL_rsimpStrong_prune_rows_acc_raw:
  "RL (RALTS (seen @ rsimpStrong_prune_rows_acc_raw seen rs)) =
    RL (RALTS (seen @ rs))"
proof (induct rs arbitrary: seen)
  case Nil
  then show ?case
...
next
  ...
  have tail:
    "RL (RALTS ((?r' # seen) @
        rsimpStrong_prune_rows_acc_raw (?r' # seen) rs)) =
      RL (RALTS ((?r' # seen) @ rs))"
    by (rule Cons.hyps)
  have head: "RL (RALTS (seen @ [?r'])) = RL (RALTS (seen @ [r]))"
    by (rule RL_rsimpStrong_prune_against_rows_raw)
  have tail_seen:
    "RL (RALTS (seen @ ?r' #
        rsimpStrong_prune_rows_acc_raw (?r' # seen) rs)) =
      RL (RALTS (seen @ ?r' # rs))"
  proof -
    have left:
      "set (seen @ ?r' # rsimpStrong_prune_rows_acc_raw (?r' # seen) rs) =
        set ((?r' # seen) @ rsimpStrong_prune_rows_acc_raw (?r' # seen) rs)"
      by auto
    have right: "set (seen @ ?r' # rs) = set ((?r' # seen) @ rs)"
      by auto
    show ?thesis
  ...
  proof -
    have start_set:
      "set (seen @ rsimpStrong_prune_rows_acc_raw seen (r # rs)) =
        set (seen @ ?r' #
          rsimpStrong_prune_rows_acc_raw (?r' # seen) rs)"
      by (simp add: Let_def)
    have "RL (RALTS (seen @ rsimpStrong_prune_rows_acc_raw seen (r # rs))) =
        RL (RALTS (seen @ ?r' #
          rsimpStrong_prune_rows_acc_raw (?r' # seen) rs))"
      by (rule RL_RALTS_set_eq[OF start_set])
    also have "... = RL (RALTS (seen @ ?r' # rs))"
...
lemma RL_rsimpStrong_prune_rows_raw:
  "RL (RALTS (rsimpStrong_prune_rows_raw rs)) = RL (RALTS rs)"
  using RL_rsimpStrong_prune_rows_acc_raw[of "[]" rs]
  by (simp add: rsimpStrong_prune_rows_raw_def)

lemma RL_rsimpStrong_ALTs_raw:
  "RL (rsimpStrong_ALTs_raw rs) = RL (RALTS rs)"
proof -
  have "RL (rsimpStrong_ALTs_raw rs) =
      RL (RALTS (rsimpStrong_prune_rows_raw rs))"
    by (simp add: rsimpStrong_ALTs_raw_def RL_rsimp_ALTs_normalize)
  also have "... = RL (RALTS rs)"
    by (rule RL_rsimpStrong_prune_rows_raw)
  finally show ?thesis .
qed

fun rsimpStrong_raw :: "rrexp \<Rightarrow> rrexp" where
  "rsimpStrong_raw RZERO = RZERO"
| "rsimpStrong_raw RONE = RONE"
| "rsimpStrong_raw (RCHAR c) = RCHAR c"
| "rsimpStrong_raw (RSEQ r1 r2) =
    rsimp7_SEQ_atom (rsimpStrong_raw r1) (rsimpStrong_raw r2)"
| "rsimpStrong_raw (RALTS rs) =
    rsimpStrong_ALTs_raw (rflts (map rsimpStrong_raw rs))"
...
lemma RL_rsimpStrong_raw:
  "RL (rsimpStrong_raw r) = RL r"
proof (induct r)
  case RZERO
  then show ?case by simp
next
...
next
  case (RALTS rs)
  have "RL (rsimpStrong_raw (RALTS rs)) =
      RL (RALTS (rflts (map rsimpStrong_raw rs)))"
    by (simp add: RL_rsimpStrong_ALTs_raw)
  also have "... = RL (RALTS (map rsimpStrong_raw rs))"
    using RL_rsimp_rflts[of "map rsimpStrong_raw rs"] by simp
  also have "... = RL (RALTS rs)"
    using RALTS by auto
  finally show ?case .
next
...
definition rpder_strong_list_raw :: "char \<Rightarrow> rrexp \<Rightarrow> rrexp list" where
  "rpder_strong_list_raw c r = map rsimpStrong_raw (rpder_norm_list c r)"

definition rpder_strong_rows_raw :: "char \<Rightarrow> rrexp list \<Rightarrow> rrexp list" where
  "rpder_strong_rows_raw c rs =
    rdistinct
      (rflts
        (rsimpStrong_prune_rows_raw
          (rflts (concat (map (rpder_strong_list_raw c) rs))))) {}"

fun rpders_strong_rows_raw :: "rrexp list \<Rightarrow> string \<Rightarrow> rrexp list" where
  "rpders_strong_rows_raw rs [] = rs"
| "rpders_strong_rows_raw rs (c # s) =
    rpders_strong_rows_raw (rpder_strong_rows_raw c rs) s"
...
lemma distinct_rpders_strong_rows_raw:
  assumes "distinct rs"
  shows "distinct (rpders_strong_rows_raw rs s)"
  using assms by (induct s arbitrary: rs) simp_all

lemma distinct_rpders_strong1_rows_raw [simp]:
  "distinct (rpders_strong1_rows_raw r s)"
  by (simp add: rpders_strong1_rows_raw_def distinct_rpders_strong_rows_raw)
...
lemma row_group_nf_map_rsimp4_SEQ_atom:
  assumes rows: "\<forall>p \<in> set xs. row_group_nf p"
      and k: "row_group_nf k"
  shows "\<forall>p \<in> set (map (\<lambda>x. rsimp4_SEQ_atom x k) xs). row_group_nf p"
  using rows
proof (induct xs)
  case Nil
  then show ?case
...
next
  ...
  have x_nf: "row_group_nf x"
    using Cons.prems by simp
  have xs_nf: "\<forall>p \<in> set xs. row_group_nf p"
    using Cons.prems by simp
  have head: "row_group_nf (rsimp4_SEQ_atom x k)"
    by (rule row_group_nf_rsimp4_SEQ_atom[OF x_nf k])
  have tail: "\<forall>p \<in> set (map (\<lambda>x. rsimp4_SEQ_atom x k) xs). row_group_nf p"
    by (rule Cons.hyps[OF xs_nf])
  show ?case
    using head tail by simp
qed

lemma row_group_nf_map_rsimpStrong:
  assumes "\<forall>p \<in> set xs. row_group_nf p"
  shows "\<forall>p \<in> set (map rsimpStrong xs). row_group_nf p"
  using assms
proof (induct xs)
  case Nil
  then show ?case
...
next
  ...
  have xs_nf: "\<forall>p \<in> set xs. row_group_nf p"
    using Cons.prems by simp
  have head: "row_group_nf (rsimpStrong x)"
    by (rule row_group_nf_rsimpStrong[OF x_nf])
  have tail: "\<forall>p \<in> set (map rsimpStrong xs). row_group_nf p"
    by (rule Cons.hyps[OF xs_nf])
  show ?case
    using head tail by simp
qed

fun row_group_deep_nf :: "rrexp \<Rightarrow> bool" where
  "row_group_deep_nf RZERO = True"
| "row_group_deep_nf RONE = True"
| "row_group_deep_nf (RCHAR c) = True"
| "row_group_deep_nf (RALTS rs) = (\<forall>r \<in> set rs. row_group_deep_nf r)"
| "row_group_deep_nf (RSEQ r1 r2) =
    (row_group_deep_nf r1 \<and> row_group_deep_nf r2 \<and>
      r1 \<noteq> RZERO \<and> r1 \<noteq> RONE \<and> r2 \<noteq> RZERO \<and> r2 \<noteq> RONE)"
| "row_group_deep_nf (RSTAR r) = row_group_deep_nf r"
| "row_group_deep_nf (RNTIMES r n) = row_group_deep_nf r"
| "row_group_deep_nf (RBACKREF4 r1 r2 r3 r4 cs) = True"
| "row_group_deep_nf (RHALF r cs rep) = True"
| "row_group_deep_nf (RRESIDUE cs rep) = True"

lemma row_group_deep_nf_imp_row_group_nf:
...
lemma row_group_deep_nf_rflts:
  ...
  shows "\<forall>x \<in> set (rflts rs). row_group_deep_nf x"
  using assms by (induct rs rule: rflts.induct) auto

lemma row_group_deep_nf_rdistinct:
  assumes "\<forall>x \<in> set rs. row_group_deep_nf x"
  shows "\<forall>x \<in> set (rdistinct rs acc). row_group_deep_nf x"
  using assms by (auto simp add: rdistinct_set_equality1)

lemma row_group_deep_nf_rsimp_ALTs:
  assumes "\<forall>x \<in> set rs. row_group_deep_nf x"
  shows "row_group_deep_nf (rsimp_ALTs rs)"
proof (cases rs)
  case Nil
  then show ?thesis
    by simp
...
lemma row_group_deep_nf_normalize:
  assumes "\<forall>r \<in> set rs. row_group_deep_nf r"
  shows "row_group_deep_nf (rsimp_ALTs (rdistinct (rflts rs) {}))"
proof -
  have flat: "\<forall>x \<in> set (rflts rs). row_group_deep_nf x"
    by (rule row_group_deep_nf_rflts[OF assms])
  have distinct: "\<forall>x \<in> set (rdistinct (rflts rs) {}). row_group_deep_nf x"
    by (rule row_group_deep_nf_rdistinct[OF flat])
  show ?thesis
    by (rule row_group_deep_nf_rsimp_ALTs[OF distinct])
qed

lemma row_group_deep_nf_rsimp4_SEQ_atom:
  assumes "row_group_deep_nf x" "row_group_deep_nf y"
  shows "row_group_deep_nf (rsimp4_SEQ_atom x y)"
  using assms
proof (induct x arbitrary: y)
  case RZERO
  then show ?case by simp
...
next
  case (RSEQ x1 x2)
  have x1_nf: "row_group_deep_nf x1"
    using RSEQ.prems by simp
  have x2_nf: "row_group_deep_nf x2"
    using RSEQ.prems by simp
  have inner: "row_group_deep_nf (rsimp4_SEQ_atom x2 y)"
    by (rule RSEQ.hyps(2)[OF x2_nf RSEQ.prems(2)])
  show ?case
    using RSEQ.hyps(1)[OF x1_nf inner] by simp
next
  case (RALTS rs)
  then show ?case
    by (cases y) simp_all
...
lemma row_group_deep_nf_rsimp7_SEQ_atom:
  assumes "row_group_deep_nf x" "row_group_deep_nf y"
  shows "row_group_deep_nf (rsimp7_SEQ_atom x y)"
proof -
  have fallback: "row_group_deep_nf (rsimp4_SEQ_atom x y)"
    by (rule row_group_deep_nf_rsimp4_SEQ_atom[OF assms])
  show ?thesis
  proof (cases x)
    case (RSTAR r)
    note x_star = RSTAR
    ...
    next
      ...
      proof (cases y1)
        case (RSTAR s)
        have y2_nf: "row_group_deep_nf y2"
          using assms y_seq by simp
        have y2_not_zero: "y2 \<noteq> RZERO"
          using assms y_seq by simp
        have y2_not_one: "y2 \<noteq> RONE"
...
lemma row_group_deep_nf_rprune_eq_against:
  assumes "\<forall>r \<in> set rs. row_group_deep_nf r"
  shows "\<forall>r \<in> set (rprune_eq_against covered rs). row_group_deep_nf r"
  using assms by (induct rs) auto

lemma row_group_deep_nf_shared_prune_result:
...
proof -
  ...
  show ?thesis
    by (rule row_group_deep_nf_rsimp7_SEQ_atom[OF normalized k])
qed

lemma row_group_deep_nf_rsimpStrong_prune_pair:
  assumes "row_group_deep_nf later"
  shows "row_group_deep_nf (rsimpStrong_prune_pair earlier later)"
proof -
  consider
    (shared) lrs rrs k where
      "earlier = RSEQ (RALTS lrs) k"
  ...
  proof cases
    ...
    show ?thesis
      using shared
      by (simp add: rsimpStrong_prune_pair_def
          row_group_deep_nf_shared_prune_result[OF rrs_nf k_nf])
  next
    case other
    have "rsimpStrong_prune_pair earlier later = later"
      using other
...
lemma row_group_deep_nf_rsimpStrong_prune_against_rows:
  assumes "row_group_deep_nf r"
  shows "row_group_deep_nf (rsimpStrong_prune_against_rows seen r)"
  using assms
proof (induct seen arbitrary: r)
  case Nil
  then show ?case by simp
next
  case (Cons x xs)
  have pair: "row_group_deep_nf (rsimpStrong_prune_pair x r)"
    by (rule row_group_deep_nf_rsimpStrong_prune_pair[OF Cons.prems])
  show ?case
    by (simp add: Cons.hyps[OF pair])
qed

lemma row_group_deep_nf_rsimpStrong_prune_rows_acc:
  assumes "\<forall>x \<in> set rs. row_group_deep_nf x"
  shows "\<forall>x \<in> set (rsimpStrong_prune_rows_acc seen rs). row_group_deep_nf x"
  using assms
proof (induct rs arbitrary: seen)
  case Nil
  then show ?case by simp
next
  ...
  have r'_nf: "row_group_deep_nf ?r'"
    by (rule row_group_deep_nf_rsimpStrong_prune_against_rows[OF r_nf])
  have tail: "\<forall>x \<in> set (rsimpStrong_prune_rows_acc (?r' # seen) rs).
      row_group_deep_nf x"
...
lemma row_group_deep_nf_rsimpStrong_prune_rows:
  assumes "\<forall>x \<in> set rs. row_group_deep_nf x"
  shows "\<forall>x \<in> set (rsimpStrong_prune_rows rs). row_group_deep_nf x"
  unfolding rsimpStrong_prune_rows_def
  by (rule row_group_deep_nf_rsimpStrong_prune_rows_acc[OF assms])

lemma row_group_deep_nf_rsimpStrong_ALTs:
  assumes "\<forall>x \<in> set rs. row_group_deep_nf x"
  shows "row_group_deep_nf (rsimpStrong_ALTs rs)"
proof -
  have pruned: "\<forall>x \<in> set (rsimpStrong_prune_rows rs). row_group_deep_nf x"
    by (rule row_group_deep_nf_rsimpStrong_prune_rows[OF assms])
  show ?thesis
    unfolding rsimpStrong_ALTs_def
    by (rule row_group_deep_nf_normalize[OF pruned])
qed

lemma row_group_deep_nf_rsimpStrong:
  assumes "row_group_deep_nf r"
  shows "row_group_deep_nf (rsimpStrong r)"
  using assms
proof (induct r)
  case RZERO
  then show ?case by simp
...
next
  case (RSEQ r1 r2)
  have left: "row_group_deep_nf (rsimpStrong r1)"
    by (rule RSEQ.hyps(1)) (use RSEQ.prems in simp)
  have right: "row_group_deep_nf (rsimpStrong r2)"
    by (rule RSEQ.hyps(2)) (use RSEQ.prems in simp)
  show ?case
    by (simp add: row_group_deep_nf_rsimp7_SEQ_atom[OF left right])
next
  case (RALTS rs)
  have elems: "\<forall>r \<in> set rs. row_group_deep_nf (rsimpStrong r)"
  proof
    fix r
    assume r: "r \<in> set rs"
    have "row_group_deep_nf r"
      using RALTS.prems r by simp
    then show "row_group_deep_nf (rsimpStrong r)"
      by (rule RALTS.hyps[OF r])
  qed
  have flat: "\<forall>x \<in> set (rflts (map rsimpStrong rs)). row_group_deep_nf x"
    by (rule row_group_deep_nf_rflts) (use elems in auto)
  show ?case
    by (simp add: row_group_deep_nf_rsimpStrong_ALTs[OF flat])
next
  case (RSTAR r)
  have inner: "row_group_deep_nf (rsimpStrong r)"
    by (rule RSTAR.hyps) (use RSTAR.prems in simp)
  show ?case
    using inner by (cases "rsimpStrong r") simp_all
next
  case (RNTIMES r n)
  then show ?case by simp
next
...
lemma row_group_deep_nf_rsimpStrong_raw_rder:
  ...
  shows "row_group_deep_nf (rsimpStrong_raw (rder c r))"
  using legacy nf
...
next
  case (RALTS rs)
  have elems: "\<forall>r \<in> set rs. row_group_deep_nf (rsimpStrong_raw (rder c r))"
  proof
    fix r
    assume r: "r \<in> set rs"
    have r_legacy: "legacy_rrexp r"
    ...
    have r_nf: "row_group_deep_nf r"
      using RALTS.prems r by simp
    show "row_group_deep_nf (rsimpStrong_raw (rder c r))"
      by (rule RALTS.hyps[OF r r_legacy r_nf])
  qed
  have flat: "\<forall>x \<in> set (rflts (map (rsimpStrong_raw \<circ> rder c) rs)).
      row_group_deep_nf x"
    by (rule row_group_deep_nf_rflts) (use elems in auto)
  show ?case
    by (simp add: row_group_deep_nf_rsimpStrong_ALTs_raw[OF flat])
next
  ...
  have r2_nf: "row_group_deep_nf r2"
    using RSEQ.prems by simp
  have der1_nf: "row_group_deep_nf (rsimpStrong_raw (rder c r1))"
    by (rule RSEQ.hyps(1)[OF r1_legacy r1_nf])
  have der2_nf: "row_group_deep_nf (rsimpStrong_raw (rder c r2))"
    by (rule RSEQ.hyps(2)[OF r2_legacy r2_nf])
  have r2_strong_nf: "row_group_deep_nf (rsimpStrong_raw r2)"
    by (rule row_group_deep_nf_rsimpStrong_raw[OF r2_nf])
  have seq_nf: "row_group_deep_nf (rsimpStrong_raw (RSEQ (rder c r1) r2))"
    by (simp add: row_group_deep_nf_rsimp7_SEQ_atom
        [OF der1_nf r2_strong_nf])
  show ?case
  proof (cases "rnullable r1")
    case True
    have mapped:
      "\<forall>x \<in> set (map rsimpStrong_raw [RSEQ (rder c r1) r2, rder c r2]).
        row_group_deep_nf x"
      using seq_nf der2_nf by simp
    have flat:
      "\<forall>x \<in> set (rflts
        (map rsimpStrong_raw [RSEQ (rder c r1) r2, rder c r2])).
        row_group_deep_nf x"
      by (rule row_group_deep_nf_rflts[OF mapped])
    have flat':
      "\<forall>x \<in> set (rflts
        [rsimp7_SEQ_atom (rsimpStrong_raw (rder c r1))
          (rsimpStrong_raw r2),
         rsimpStrong_raw (rder c r2)]).
        row_group_deep_nf x"
      using flat by simp
    have alts:
      "row_group_deep_nf (rsimpStrong_ALTs_raw
        (rflts
          [rsimp7_SEQ_atom (rsimpStrong_raw (rder c r1))
            (rsimpStrong_raw r2),
           rsimpStrong_raw (rder c r2)]))"
      by (rule row_group_deep_nf_rsimpStrong_ALTs_raw[OF flat'])
    show ?thesis
      using True
      by (simp add: alts)
  next
...
next
  ...
  have der_nf: "row_group_deep_nf (rsimpStrong_raw (rder c r))"
    by (rule RSTAR.hyps[OF r_legacy r_nf])
  have body_nf: "row_group_deep_nf (rsimpStrong_raw r)"
    by (rule row_group_deep_nf_rsimpStrong_raw) (use r_nf in simp)
  show ?case
    using der_nf body_nf
    by (cases "rsimpStrong_raw r")
      (simp_all add: row_group_deep_nf_rsimp7_SEQ_atom)
next
  case (RNTIMES r n)
  show ?case
  proof (cases n)
...
lemma row_group_deep_nf_map_rsimp4_SEQ_atom:
  assumes rows: "\<forall>p \<in> set xs. row_group_deep_nf p"
      and k: "row_group_deep_nf k"
  shows "\<forall>p \<in> set (map (\<lambda>x. rsimp4_SEQ_atom x k) xs). row_group_deep_nf p"
  using rows
proof (induct xs)
  case Nil
  then show ?case
...
next
  case (Cons x xs)
  have x_nf: "row_group_deep_nf x"
    using Cons.prems by simp
  have xs_nf: "\<forall>p \<in> set xs. row_group_deep_nf p"
    using Cons.prems by simp
  have head: "row_group_deep_nf (rsimp4_SEQ_atom x k)"
    by (rule row_group_deep_nf_rsimp4_SEQ_atom[OF x_nf k])
  have tail: "\<forall>p \<in> set (map (\<lambda>x. rsimp4_SEQ_atom x k) xs). row_group_deep_nf p"
    by (rule Cons.hyps[OF xs_nf])
  show ?case
    using head tail by simp
qed

lemma row_group_deep_nf_map_rsimpStrong:
  assumes "\<forall>p \<in> set xs. row_group_deep_nf p"
  shows "\<forall>p \<in> set (map rsimpStrong xs). row_group_deep_nf p"
  using assms
proof (induct xs)
  case Nil
  then show ?case
...
next
  ...
  have xs_nf: "\<forall>p \<in> set xs. row_group_deep_nf p"
    using Cons.prems by simp
  have head: "row_group_deep_nf (rsimpStrong x)"
    by (rule row_group_deep_nf_rsimpStrong[OF x_nf])
  have tail: "\<forall>p \<in> set (map rsimpStrong xs). row_group_deep_nf p"
    by (rule Cons.hyps[OF xs_nf])
  show ?case
    using head tail by simp
qed

lemma row_group_deep_nf_rpder_list:
  assumes "row_group_deep_nf r"
  shows "\<forall>p \<in> set (rpder_list c r). row_group_deep_nf p"
  using assms
proof (induct r)
  case RZERO
  then show ?case by simp
...
next
  ...
  have r2_nf: "row_group_deep_nf r2"
    using RSEQ.prems by simp
  have left_raw: "\<forall>p \<in> set (rpder_list c r1). row_group_deep_nf p"
    by (rule RSEQ.hyps(1)[OF r1_nf])
  have left:
    "\<forall>p \<in> set (map (\<lambda>p. rsimp4_SEQ_atom p r2) (rpder_list c r1)).
      row_group_deep_nf p"
    by (rule row_group_deep_nf_map_rsimp4_SEQ_atom[OF left_raw r2_nf])
  have right: "\<forall>p \<in> set (if rnullable r1 then rpder_list c r2 else []).
      row_group_deep_nf p"
    using RSEQ.hyps(2)[OF r2_nf] by (cases "rnullable r1") simp_all
  show ?case
  proof
    fix p
    assume p: "p \<in> set (rpder_list c (RSEQ r1 r2))"
    have split: "p \<in> set (map (\<lambda>p. rsimp4_SEQ_atom p r2) (rpder_list c r1)) \<or>
        p \<in> set (if rnullable r1 then rpder_list c r2 else [])"
      using p by simp
    show "row_group_deep_nf p"
    proof (rule disjE[OF split])
      assume "p \<in> set (map (\<lambda>p. rsimp4_SEQ_atom p r2) (rpder_list c r1))"
      then show ?thesis
        using left by blast
    next
      assume "p \<in> set (if rnullable r1 then rpder_list c r2 else [])"
      then show ?thesis
        using right by simp
    qed
  qed
next
  ...
  have r_nf: "row_group_deep_nf r"
    using RSTAR.prems by simp
  have body: "\<forall>p \<in> set (rpder_list c r). row_group_deep_nf p"
    by (rule RSTAR.hyps[OF r_nf])
  have tail: "row_group_deep_nf (RSTAR r)"
    using r_nf by simp
  show ?case
    using row_group_deep_nf_map_rsimp4_SEQ_atom[OF body tail] by simp
next
  case (RNTIMES r n)
  show ?case
  proof (cases n)
  ...
  next
    ...
    have r_nf: "row_group_deep_nf r"
      using RNTIMES.prems by simp
    have body: "\<forall>p \<in> set (rpder_list c r). row_group_deep_nf p"
      by (rule RNTIMES.hyps[OF r_nf])
    have tail: "row_group_deep_nf (RNTIMES r (n - 1))"
      using r_nf by simp
    show ?thesis
      using Suc
      by (simp add: row_group_deep_nf_map_rsimp4_SEQ_atom[OF body tail])
  qed
next
  case (RBACKREF4 r1 r2 r3 r4 x5)
  show ?case by simp
...
lemma row_group_deep_nf_rpder_norm_list:
  assumes "row_group_deep_nf r"
  shows "\<forall>p \<in> set (rpder_norm_list c r). row_group_deep_nf p"
proof -
  have raw: "\<forall>p \<in> set (rpder_list c r). row_group_deep_nf p"
    by (rule row_group_deep_nf_rpder_list[OF assms])
  have one: "row_group_deep_nf RONE"
    by simp
  show ?thesis
    unfolding rpder_norm_list_def
    by (rule row_group_deep_nf_map_rsimp4_SEQ_atom[OF raw one])
qed

lemma row_group_deep_nf_rpder_strong_list:
  assumes "row_group_deep_nf r"
  shows "\<forall>p \<in> set (rpder_strong_list c r). row_group_deep_nf p"
proof -
  have rows: "\<forall>p \<in> set (rpder_norm_list c r). row_group_deep_nf p"
    by (rule row_group_deep_nf_rpder_norm_list[OF assms])
  show ?thesis
    unfolding rpder_strong_list_def
    by (rule row_group_deep_nf_map_rsimpStrong[OF rows])
qed

lemma row_group_deep_nf_concat_map_rpder_strong_list:
  assumes "\<forall>r \<in> set rs. row_group_deep_nf r"
  shows "\<forall>p \<in> set (concat (map (rpder_strong_list c) rs)). row_group_deep_nf p"
  using assms
proof (induct rs)
  case Nil
  then show ?case by simp
next
  ...
  have r_nf: "row_group_deep_nf r"
    using Cons.prems by simp
  have rs_nf: "\<forall>q \<in> set rs. row_group_deep_nf q"
    using Cons.prems by simp
  have head: "\<forall>p \<in> set (rpder_strong_list c r). row_group_deep_nf p"
    by (rule row_group_deep_nf_rpder_strong_list[OF r_nf])
  have tail: "\<forall>p \<in> set (concat (map (rpder_strong_list c) rs)).
      row_group_deep_nf p"
    by (rule Cons.hyps[OF rs_nf])
  show ?case
  proof
    fix p
    assume p: "p \<in> set (concat (map (rpder_strong_list c) (r # rs)))"
    have split: "p \<in> set (rpder_strong_list c r) \<or>
        p \<in> set (concat (map (rpder_strong_list c) rs))"
      using p by simp
    show "row_group_deep_nf p"
    proof (rule disjE[OF split])
      assume "p \<in> set (rpder_strong_list c r)"
      then show ?thesis
        using head by simp
    next
      assume "p \<in> set (concat (map (rpder_strong_list c) rs))"
      then show ?thesis
        using tail by blast
    qed
  qed
...
lemma row_group_deep_nf_rpder_strong_rows:
  assumes "\<forall>r \<in> set rs. row_group_deep_nf r"
  shows "\<forall>p \<in> set (rpder_strong_rows c rs). row_group_deep_nf p"
proof -
  have mapped:
    "\<forall>p \<in> set (concat (map (rpder_strong_list c) rs)). row_group_deep_nf p"
    by (rule row_group_deep_nf_concat_map_rpder_strong_list[OF assms])
  have flat1:
    "\<forall>p \<in> set (rflts (concat (map (rpder_strong_list c) rs))).
      row_group_deep_nf p"
    by (rule row_group_deep_nf_rflts[OF mapped])
  have pruned:
    "\<forall>p \<in> set (rsimpStrong_prune_rows
      (rflts (concat (map (rpder_strong_list c) rs)))).
      row_group_deep_nf p"
    by (rule row_group_deep_nf_rsimpStrong_prune_rows[OF flat1])
  have flat2:
    "\<forall>p \<in> set (rflts (rsimpStrong_prune_rows
      (rflts (concat (map (rpder_strong_list c) rs))))).
      row_group_deep_nf p"
...
lemma row_group_deep_nf_rpders_strong_rows:
  assumes "\<forall>r \<in> set rs. row_group_deep_nf r"
      and "p \<in> set (rpders_strong_rows rs s)"
...
next
  case (Cons c s)
  have next_nf: "\<forall>r \<in> set (rpder_strong_rows c rs). row_group_deep_nf r"
    by (rule row_group_deep_nf_rpder_strong_rows[OF Cons.prems(1)])
  have p_next: "p \<in> set (rpders_strong_rows (rpder_strong_rows c rs) s)"
    using Cons.prems(2) by simp
  show ?case
...
lemma row_group_deep_nf_rpders_strong1_rows:
  assumes "row_group_deep_nf r"
      and "p \<in> set (rpders_strong1_rows r s)"
  shows "row_group_deep_nf p"
  using assms row_group_deep_nf_rpders_strong_rows[of "[r]" p s]
  by (simp add: rpders_strong1_rows_def)

lemma row_group_nf_rpder_strong_rows:
  assumes "\<forall>r \<in> set rs. row_group_deep_nf r"
  shows "\<forall>p \<in> set (rpder_strong_rows c rs). row_group_nf p"
proof
  fix p
  assume p: "p \<in> set (rpder_strong_rows c rs)"
  have deep: "\<forall>q \<in> set (rpder_strong_rows c rs). row_group_deep_nf q"
    by (rule row_group_deep_nf_rpder_strong_rows[OF assms])
  have "row_group_deep_nf p"
    using deep p by simp
  then show "row_group_nf p"
    by (rule row_group_deep_nf_imp_row_group_nf)
qed

lemma row_group_nf_rpders_strong_rows:
  assumes "\<forall>r \<in> set rs. row_group_deep_nf r"
      and "p \<in> set (rpders_strong_rows rs s)"
  shows "row_group_nf p"
proof -
  have "row_group_deep_nf p"
    by (rule row_group_deep_nf_rpders_strong_rows[OF assms])
  then show ?thesis
    by (rule row_group_deep_nf_imp_row_group_nf)
qed

lemma row_group_nf_rpders_strong1_rows:
  assumes "row_group_deep_nf r"
      and "p \<in> set (rpders_strong1_rows r s)"
  shows "row_group_nf p"
proof -
...
lemma rpder_strong_rows_norm_later_shared_subsetI:
  assumes norm: "\<And>q p. q \<in> set rs \<Longrightarrow>
      p \<in> set (rpder_norm_list c q) \<Longrightarrow>
      set (rflts [rsimpStrong p]) \<subseteq> U"
    and flat_closed: "\<And>q. q \<in> U \<Longrightarrow> set (rflts [q]) \<subseteq> U"
...
next
  ...
  assume xs: "set xs \<subseteq> U"
  show "set (rflts (rsimpStrong_prune_rows xs)) \<subseteq> U"
  proof (rule rsimpStrong_prune_rows_later_shared_subsetI)
    fix r
    assume r: "r \<in> set xs"
    show "set (rflts [r]) \<subseteq> U"
      by (rule flat_closed) (use xs r in blast)
  next
    ...
    show "set (rflts [rsimp7_SEQ_atom
      (rsimp_ALTs (rdistinct (rflts (rprune_eq_against lrs rrs)) {}))
      k]) \<subseteq> U"
      by (rule shared[OF later])
  qed
qed

lemma rpders_strong_rows_norm_later_shared_subsetI:
  assumes init: "set rs \<subseteq> U"
    and flat_closed: "\<And>q. q \<in> U \<Longrightarrow> set (rflts [q]) \<subseteq> U"
    and norm: "\<And>xs c q p. set xs \<subseteq> U \<Longrightarrow>
      q \<in> set xs \<Longrightarrow> p \<in> set (rpder_norm_list c q) \<Longrightarrow>
...
proof (rule rpders_strong_rows_subsetI[OF init])
  ...
  assume xs: "set xs \<subseteq> U"
  show "set (rpder_strong_rows c xs) \<subseteq> U"
  proof (rule rpder_strong_rows_norm_later_shared_subsetI)
    fix q p
    assume q: "q \<in> set xs"
      and p: "p \<in> set (rpder_norm_list c q)"
    show "set (rflts [rsimpStrong p]) \<subseteq> U"
...
lemma rpders_strong_rows_raw_subsetI:
  assumes init: "set rs \<subseteq> U"
      and step: "\<And>xs c. set xs \<subseteq> U \<Longrightarrow>
...
next
  case (Cons c s)
  have next_rows: "set (rpder_strong_rows_raw c rs) \<subseteq> U"
    by (rule step[OF Cons.prems])
  show ?case
    by (simp add: Cons.hyps[OF next_rows])
qed

lemma rpders_strong1_rows_raw_subsetI:
  assumes init: "r \<in> U"
      and step: "\<And>xs c. set xs \<subseteq> U \<Longrightarrow>
        set (rpder_strong_rows_raw c xs) \<subseteq> U"
  shows "set (rpders_strong1_rows_raw r s) \<subseteq> U"
proof -
  have rows: "set [r] \<subseteq> U"
    using init by simp
  show ?thesis
    by (simp add: rpders_strong1_rows_raw_def
        rpders_strong_rows_raw_subsetI[OF rows step])
qed

lemma rsimpStrong_prune_pair_raw_shared_subsetI:
  assumes later: "set (rflts [later]) \<subseteq> U"
    and shared: "\<And>lrs rrs k.
      earlier = RSEQ (RALTS lrs) k \<Longrightarrow>
      later = RSEQ (RALTS rrs) k \<Longrightarrow>
...
lemma rsimpStrong_prune_against_rows_raw_pair_subsetI:
  assumes row: "set (rflts [r]) \<subseteq> U"
    and pair: "\<And>earlier later. earlier \<in> set seen \<Longrightarrow>
      set (rflts [later]) \<subseteq> U \<Longrightarrow>
...
lemma rsimpStrong_prune_rows_acc_raw_pair_subsetI:
  assumes rows: "\<And>r. r \<in> set rs \<Longrightarrow> set (rflts [r]) \<subseteq> U"
    and pair: "\<And>earlier later.
      set (rflts [later]) \<subseteq> U \<Longrightarrow>
      set (rflts [rsimpStrong_prune_pair_raw earlier later]) \<subseteq> U"
...
next
  ...
  let ?r' = "rsimpStrong_prune_against_rows_raw seen r"
  have head: "set (rflts [?r']) \<subseteq> U"
  proof (rule rsimpStrong_prune_against_rows_raw_pair_subsetI)
    show "set (rflts [r]) \<subseteq> U"
      by (rule Cons.prems) simp
    show "\<And>earlier later. earlier \<in> set seen \<Longrightarrow>
      set (rflts [later]) \<subseteq> U \<Longrightarrow>
...
lemma rsimpStrong_prune_rows_raw_pair_subsetI:
  assumes rows: "\<And>r. r \<in> set rs \<Longrightarrow> set (rflts [r]) \<subseteq> U"
    and pair: "\<And>earlier later.
      set (rflts [later]) \<subseteq> U \<Longrightarrow>
      set (rflts [rsimpStrong_prune_pair_raw earlier later]) \<subseteq> U"
  ...
  unfolding rsimpStrong_prune_rows_raw_def
  by (rule rsimpStrong_prune_rows_acc_raw_pair_subsetI[OF rows pair])

lemma rsimpStrong_prune_pair_raw_later_shared_subsetI:
  assumes later: "set (rflts [later]) \<subseteq> U"
    and shared: "\<And>lrs rrs k.
      RSEQ (RALTS rrs) k \<in> U \<Longrightarrow>
      set (rflts [rsimp7_SEQ_atom
...
lemma rsimpStrong_prune_rows_raw_later_shared_subsetI:
  assumes rows: "\<And>r. r \<in> set rs \<Longrightarrow> set (rflts [r]) \<subseteq> U"
    and shared: "\<And>lrs rrs k.
      RSEQ (RALTS rrs) k \<in> U \<Longrightarrow>
      set (rflts [rsimp7_SEQ_atom
        (rsimp_ALTs (rprune_eq_against lrs rrs)) k]) \<subseteq> U"
  shows "set (rflts (rsimpStrong_prune_rows_raw rs)) \<subseteq> U"
proof (rule rsimpStrong_prune_rows_raw_pair_subsetI[OF rows])
  fix earlier later
  assume later: "set (rflts [later]) \<subseteq> U"
  show "set (rflts [rsimpStrong_prune_pair_raw earlier later]) \<subseteq> U"
    by (rule rsimpStrong_prune_pair_raw_later_shared_subsetI
        [OF later shared])
qed

lemma rflts_rpder_strong_list_raw_subsetI:
  assumes "\<And>p. p \<in> set (rpder_norm_list c q) \<Longrightarrow>
    set (rflts [rsimpStrong_raw p]) \<subseteq> U"
  shows "set (rflts (rpder_strong_list_raw c q)) \<subseteq> U"
proof
  fix x
  assume x: "x \<in> set (rflts (rpder_strong_list_raw c q))"
  have x_map: "x \<in> set (rflts (map rsimpStrong_raw (rpder_norm_list c q)))"
    using x by (simp add: rpder_strong_list_raw_def)
  obtain p where p: "p \<in> set (rpder_norm_list c q)"
      and x_p: "x \<in> set (rflts [rsimpStrong_raw p])"
    by (rule set_rflts_map_memberE[OF x_map])
  have "set (rflts [rsimpStrong_raw p]) \<subseteq> U"
    by (rule assms[OF p])
  then show "x \<in> U"
    using x_p by blast
qed

lemma rflts_concat_map_rpder_strong_list_raw_subsetI:
  assumes "\<And>q. q \<in> set rs \<Longrightarrow>
    set (rflts (rpder_strong_list_raw c q)) \<subseteq> U"
  shows "set (rflts (concat (map (rpder_strong_list_raw c) rs))) \<subseteq> U"
proof
  fix x
  assume x: "x \<in> set (rflts (concat (map (rpder_strong_list_raw c) rs)))"
  have "set (rflts (concat (map (rpder_strong_list_raw c) rs))) \<subseteq>
      (\<Union>q \<in> set rs. set (rflts (rpder_strong_list_raw c q)))"
    by (induct rs) (auto simp add: flts_append)
  then show "x \<in> U"
    using x assms by blast
qed

lemma rpder_strong_rows_raw_local_subsetI:
  assumes lists: "\<And>q. q \<in> set rs \<Longrightarrow>
      set (rflts (rpder_strong_list_raw c q)) \<subseteq> U"
    and prune: "\<And>xs. set xs \<subseteq> U \<Longrightarrow>
      set (rflts (rsimpStrong_prune_rows_raw xs)) \<subseteq> U"
  shows "set (rpder_strong_rows_raw c rs) \<subseteq> U"
proof -
  let ?rows = "rflts (concat (map (rpder_strong_list_raw c) rs))"
  have flat: "set ?rows \<subseteq> U"
    by (rule rflts_concat_map_rpder_strong_list_raw_subsetI[OF lists])
  have pruned: "set (rflts (rsimpStrong_prune_rows_raw ?rows)) \<subseteq> U"
    by (rule prune[OF flat])
  have "set (rdistinct (rflts (rsimpStrong_prune_rows_raw ?rows)) {}) \<subseteq> U"
    by (rule set_rdistinct_subset[OF pruned])
  then show ?thesis
    by (simp add: rpder_strong_rows_raw_def)
qed

lemma rpder_strong_rows_raw_norm_later_shared_subsetI:
  assumes norm: "\<And>q p. q \<in> set rs \<Longrightarrow>
      p \<in> set (rpder_norm_list c q) \<Longrightarrow>
      set (rflts [rsimpStrong_raw p]) \<subseteq> U"
    and flat_closed: "\<And>q. q \<in> U \<Longrightarrow> set (rflts [q]) \<subseteq> U"
...
next
  ...
  proof (rule rsimpStrong_prune_rows_raw_later_shared_subsetI)
    fix r
    assume r: "r \<in> set xs"
    show "set (rflts [r]) \<subseteq> U"
      by (rule flat_closed) (use xs r in blast)
...
lemma rpders_strong_rows_raw_norm_later_shared_subsetI:
  assumes init: "set rs \<subseteq> U"
    and flat_closed: "\<And>q. q \<in> U \<Longrightarrow> set (rflts [q]) \<subseteq> U"
    and norm: "\<And>xs c q p. set xs \<subseteq> U \<Longrightarrow>
      q \<in> set xs \<Longrightarrow> p \<in> set (rpder_norm_list c q) \<Longrightarrow>
...
proof (rule rpders_strong_rows_raw_subsetI[OF init])
  ...
  proof (rule rpder_strong_rows_raw_norm_later_shared_subsetI)
    fix q p
    assume q: "q \<in> set xs"
      and p: "p \<in> set (rpder_norm_list c q)"
...
definition raw_shared_prune_same_suffix_pairs ::
...
definition raw_shared_prune_suffix_bucket ::
  "rrexp set \<Rightarrow> rrexp option \<Rightarrow> rrexp set" where
  "raw_shared_prune_suffix_bucket U k =
    {q \<in> U. raw_shared_prune_suffix_key q = k}"
...
proof -
  have "raw_shared_prune_same_suffix_pairs U \<subseteq> U \<times> U"
...
definition raw_final_active_suffix_alt_nodes :: "rrexp \<Rightarrow> rrexp set" where
  "raw_final_active_suffix_alt_nodes r =
    {RALTS rows | rows k.
      RSEQ (RALTS rows) k \<in> raw_final_active_suffix_rows r}"
...
definition raw_final_active_suffix_payload_dag_universe ::
  ...
  "raw_final_active_suffix_payload_dag_universe r =
    {p. \<exists>rows k q.
      RSEQ (RALTS rows) k \<in> raw_final_active_suffix_rows r \<and>
      q \<in> set rows \<and> p \<in> rsubterms q}"

definition raw_final_active_suffix_key_dag_universe ::
  "rrexp \<Rightarrow> rrexp set" where
  "raw_final_active_suffix_key_dag_universe r =
    (\<Union>k\<in>raw_final_active_suffix_keys r. rsubterms k)"

lemma raw_final_active_suffix_alt_nodes_subset_rsubterms:
  "raw_final_active_suffix_alt_nodes r \<subseteq> rsubterms r"
proof
  fix p
...
lemma raw_final_active_suffix_payload_dag_universe_subset_rsubterms:
  "raw_final_active_suffix_payload_dag_universe r \<subseteq> rsubterms r"
proof
  fix p
  ...
  proof -
    have "q \<in> (\<Union> (set (map rsubterms rows)))"
      using q self_rsubterm by force
    then show ?thesis by simp
  qed
  have q_sub: "q \<in> rsubterms (RSEQ (RALTS rows) k)"
    using q_alt by auto
  have "q \<in> rsubterms r"
    by (rule rsubterms_trans[OF row_sub q_sub])
  then show "p \<in> rsubterms r"
    by (rule rsubterms_trans[OF _ p_sub])
qed

lemma raw_final_active_suffix_key_dag_universe_subset_rsubterms:
  "raw_final_active_suffix_key_dag_universe r \<subseteq> rsubterms r"
proof
  fix p
...
lemma finite_raw_final_active_suffix_alt_nodes [simp]:
  "finite (raw_final_active_suffix_alt_nodes r)"
...
lemma finite_raw_final_active_suffix_payload_dag_universe [simp]:
  ...
  by (rule finite_subset
      [OF raw_final_active_suffix_payload_dag_universe_subset_rsubterms])
    simp

lemma finite_raw_final_active_suffix_key_dag_universe [simp]:
  "finite (raw_final_active_suffix_key_dag_universe r)"
  by (rule finite_subset
...
lemma raw_final_active_suffix_row_dag_universe_decomp_subset:
  "raw_final_active_suffix_row_dag_universe r \<subseteq>
    ...
    raw_final_active_suffix_alt_nodes r \<union>
    raw_final_active_suffix_payload_dag_universe r \<union>
    raw_final_active_suffix_key_dag_universe r"
proof
  fix p
  assume p: "p \<in> raw_final_active_suffix_row_dag_universe r"
  then obtain q rows k where q_def: "q = RSEQ (RALTS rows) k"
    and row: "q \<in> raw_final_active_suffix_rows r"
    and p_sub: "p \<in> rsubterms q"
    by (elim raw_final_active_suffix_row_dag_universeE)
  have row_sub: "q \<in> rsubterms r"
    using row raw_final_active_suffix_rows_subset_rsubterms by blast
  have row_shape_sub: "RSEQ (RALTS rows) k \<in> rsubterms r"
    using q_def row_sub by simp
  have key: "k \<in> raw_final_active_suffix_keys r"
    using row_shape_sub raw_final_active_suffix_keys_iff by blast
  show "p \<in> raw_final_active_suffix_rows r \<union>
      raw_final_active_suffix_alt_nodes r \<union>
      raw_final_active_suffix_payload_dag_universe r \<union>
      raw_final_active_suffix_key_dag_universe r"
    using row p_sub key q_def
    by (auto simp add: raw_final_active_suffix_alt_nodes_def
        raw_final_active_suffix_payload_dag_universe_def
        raw_final_active_suffix_key_dag_universe_def)
qed

lemma card_raw_final_active_suffix_row_dag_universe_decomp:
  "card (raw_final_active_suffix_row_dag_universe r) \<le>
    card (raw_final_active_suffix_rows r) +
    card (raw_final_active_suffix_alt_nodes r) +
    card (raw_final_active_suffix_payload_dag_universe r) +
    card (raw_final_active_suffix_key_dag_universe r)"
proof -
  let ?Rows = "raw_final_active_suffix_rows r"
  let ?Alts = "raw_final_active_suffix_alt_nodes r"
  let ?Payload = "raw_final_active_suffix_payload_dag_universe r"
  let ?Keys = "raw_final_active_suffix_key_dag_universe r"
  let ?Parts = "?Rows \<union> ?Alts \<union> ?Payload \<union> ?Keys"
  have "card (raw_final_active_suffix_row_dag_universe r) \<le> card ?Parts"
    by (rule card_mono)
      (simp_all add: raw_final_active_suffix_row_dag_universe_decomp_subset)
  also have "card ?Parts \<le>
      card ?Rows + card ?Alts + card ?Payload + card ?Keys"
  proof -
    have a: "card ?Parts \<le> card (?Rows \<union> ?Alts \<union> ?Payload) +
        card ?Keys"
      by (rule card_Un_le)
    have b: "card (?Rows \<union> ?Alts \<union> ?Payload) \<le>
        card (?Rows \<union> ?Alts) + card ?Payload"
      by (rule card_Un_le)
    have c: "card (?Rows \<union> ?Alts) \<le> card ?Rows + card ?Alts"
      by (rule card_Un_le)
    show ?thesis
      using a b c by linarith
  qed
  finally show ?thesis .
qed

lemma card_raw_final_active_suffix_row_dag_universe_decomp_boundI:
  assumes rows: "card (raw_final_active_suffix_rows r) \<le> R"
    and alts: "card (raw_final_active_suffix_alt_nodes r) \<le> A"
    and payload:
...
lemma finite_raw_final_active_suffix_closure [simp]:
  "finite (raw_final_active_suffix_closure r)"
...
lemma raw_shared_prune_same_suffix_pairs_bucket_union:
  "raw_shared_prune_same_suffix_pairs U =
    (\<Union>k\<in>raw_shared_prune_suffix_key ` U.
      raw_shared_prune_suffix_bucket U k \<times>
      raw_shared_prune_suffix_bucket U k)"
  by (auto simp add: raw_shared_prune_same_suffix_pairs_def
...
lemma card_raw_shared_prune_same_suffix_pairs_le_sum_buckets:
  assumes finite: "finite U"
  shows "card (raw_shared_prune_same_suffix_pairs U) \<le>
    (\<Sum>k\<in>raw_shared_prune_suffix_key ` U.
      card (raw_shared_prune_suffix_bucket U k) *
      card (raw_shared_prune_suffix_bucket U k))"
proof -
  let ?keys = "raw_shared_prune_suffix_key ` U"
  have keys_finite: "finite ?keys"
    using finite by simp
  have "card (raw_shared_prune_same_suffix_pairs U) =
      card (\<Union>k\<in>?keys.
        raw_shared_prune_suffix_bucket U k \<times>
        raw_shared_prune_suffix_bucket U k)"
    by (simp add: raw_shared_prune_same_suffix_pairs_bucket_union)
  also have "... \<le> (\<Sum>k\<in>?keys.
      card (raw_shared_prune_suffix_bucket U k \<times>
        raw_shared_prune_suffix_bucket U k))"
    by (rule card_UN_le[OF keys_finite])
  also have "... =
      (\<Sum>k\<in>?keys.
...
lemma card_raw_shared_prune_same_suffix_pairs_bucket_bound:
  assumes finite: "finite U"
  ...
  shows "card (raw_shared_prune_same_suffix_pairs U) \<le> S * K * K"
proof -
  let ?keys = "raw_shared_prune_suffix_key ` U"
  have pair_sum:
    "card (raw_shared_prune_same_suffix_pairs U) \<le>
      (\<Sum>k\<in>?keys.
        card (raw_shared_prune_suffix_bucket U k) *
        card (raw_shared_prune_suffix_bucket U k))"
    by (rule card_raw_shared_prune_same_suffix_pairs_le_sum_buckets[OF finite])
  have "(\<Sum>k\<in>?keys.
        card (raw_shared_prune_suffix_bucket U k) *
        card (raw_shared_prune_suffix_bucket U k))
  ...
  proof (rule sum_mono)
    fix k
    assume k: "k \<in> ?keys"
    have bucket_le: "card (raw_shared_prune_suffix_bucket U k) \<le> K"
      by (rule bucket_bound[OF k])
    show "card (raw_shared_prune_suffix_bucket U k) *
        card (raw_shared_prune_suffix_bucket U k) \<le> K * K"
  ...
  also have "... \<le> S * (K * K)"
    ...
    "(\<Sum>k\<in>?keys.
        card (raw_shared_prune_suffix_bucket U k) *
        card (raw_shared_prune_suffix_bucket U k)) \<le> S * (K * K)" .
  then have "card (raw_shared_prune_same_suffix_pairs U) \<le> S * (K * K)"
...
lemma card_raw_shared_prune_same_suffix_closure_bound:
  assumes finite: "finite U"
    and output_bound: "\<And>earlier later.
      (earlier, later) \<in> raw_shared_prune_same_suffix_pairs U \<Longrightarrow>
      card (raw_shared_prune_pair_outputs earlier later) \<le> Out"
  shows "card (raw_shared_prune_same_suffix_closure U) \<le>
    card U + card (raw_shared_prune_same_suffix_pairs U) * Out"
proof -
  let ?P = "raw_shared_prune_same_suffix_pairs U"
  let ?F = "\<lambda>(earlier, later). raw_shared_prune_pair_outputs earlier later"
  have finP: "finite ?P"
    using finite by simp
  ...
  qed
  have "card (raw_shared_prune_same_suffix_closure U) \<le>
      card U + card (\<Union>p\<in>?P. ?F p)"
    by (simp add: raw_shared_prune_same_suffix_closure_as_pairs card_Un_le)
...
proof -
  have pairs_bound:
    "card (raw_shared_prune_same_suffix_pairs U) \<le> S * K * K"
    by (rule card_raw_shared_prune_same_suffix_pairs_bucket_bound
        [OF finite keys_bound bucket_bound])
  have "card (raw_shared_prune_same_suffix_closure U) \<le>
      card U + card (raw_shared_prune_same_suffix_pairs U) * Out"
    by (rule card_raw_shared_prune_same_suffix_closure_bound
        [OF finite output_bound])
  also have "... \<le> card U + (S * K * K) * Out"
  proof -
    have "card (raw_shared_prune_same_suffix_pairs U) * Out \<le>
        (S * K * K) * Out"
...
lemma length_rpders_strong_rows_raw_finite_universe_boundI:
  assumes init: "set rs \<subseteq> U"
      ...
      and finite: "finite U"
      and distinct: "distinct rs"
  shows "length (rpders_strong_rows_raw rs s) \<le> card U"
proof -
  have rows: "set (rpders_strong_rows_raw rs s) \<subseteq> U"
    by (rule rpders_strong_rows_raw_subsetI[OF init step])
  have dist: "distinct (rpders_strong_rows_raw rs s)"
    by (rule distinct_rpders_strong_rows_raw[OF distinct])
  show ?thesis
    by (rule length_distinct_subset_card[OF finite rows dist])
qed

lemma length_rpders_strong1_rows_raw_finite_universe_boundI:
  assumes init: "r \<in> U"
      and step: "\<And>xs c. set xs \<subseteq> U \<Longrightarrow>
        set (rpder_strong_rows_raw c xs) \<subseteq> U"
      and finite: "finite U"
  shows "length (rpders_strong1_rows_raw r s) \<le> card U"
proof -
  have rows: "set [r] \<subseteq> U"
    using init by simp
  have distinct_rows: "distinct [r]"
    by simp
  have "length (rpders_strong_rows_raw [r] s) \<le> card U"
    by (rule length_rpders_strong_rows_raw_finite_universe_boundI
        [OF rows step finite distinct_rows])
  then show ?thesis
    by (simp add: rpders_strong1_rows_raw_def)
qed

lemma rsizes_rpders_strong_rows_raw_finite_universe_boundI:
  assumes init: "set rs \<subseteq> U"
      and step: "\<And>xs c. set xs \<subseteq> U \<Longrightarrow>
        set (rpder_strong_rows_raw c xs) \<subseteq> U"
      and finite: "finite U"
      and member_size: "\<And>q. q \<in> U \<Longrightarrow> rsize q \<le> M"
      and distinct: "distinct rs"
  shows "rsizes (rpders_strong_rows_raw rs s) \<le> card U * M"
proof -
  have rows: "set (rpders_strong_rows_raw rs s) \<subseteq> U"
    by (rule rpders_strong_rows_raw_subsetI[OF init step])
  have dist: "distinct (rpders_strong_rows_raw rs s)"
...
lemma rsizes_rpders_strong1_rows_raw_finite_universe_boundI:
  assumes init: "r \<in> U"
      and step: "\<And>xs c. set xs \<subseteq> U \<Longrightarrow>
        set (rpder_strong_rows_raw c xs) \<subseteq> U"
      and finite: "finite U"
...
lemma rsizes_rpders_strong1_rows_raw_cubic_universe_boundI:
  assumes init: "r \<in> U"
      and step: "\<And>xs c. set xs \<subseteq> U \<Longrightarrow>
        set (rpder_strong_rows_raw c xs) \<subseteq> U"
      and finite: "finite U"
...
proof -
  have "rsizes (rpders_strong1_rows_raw r s) \<le> card U * M"
    by (rule rsizes_rpders_strong1_rows_raw_finite_universe_boundI
        [OF init step finite member_size])
  also have "... \<le> C * M"
    by (rule mult_right_mono[OF card_bound]) simp
  also have "... \<le> B"
...
proof -
  ...
  have step:
    "\<And>xs c. set xs \<subseteq> U \<Longrightarrow>
      set (rpder_strong_rows_raw c xs) \<subseteq> U"
    by (rule rpder_strong_rows_raw_norm_later_shared_subsetI)
      (use flat_closed norm shared in blast)+
  have "rsizes (rpders_strong_rows_raw [r] s) \<le> card U * M"
    by (rule rsizes_rpders_strong_rows_raw_finite_universe_boundI
...
lemma rsizes_rpders_strong1_rows_raw_norm_later_shared_cubic_universe_boundI:
  assumes init: "r \<in> U"
    and flat_closed: "\<And>q. q \<in> U \<Longrightarrow> set (rflts [q]) \<subseteq> U"
    and norm: "\<And>xs c q p. set xs \<subseteq> U \<Longrightarrow>
      q \<in> set xs \<Longrightarrow> p \<in> set (rpder_norm_list c q) \<Longrightarrow>
...
lemma rsizes_rpders_strong_rows_norm_later_shared_finite_universe_boundI:
  assumes init: "set rs \<subseteq> U"
    and flat_closed: "\<And>q. q \<in> U \<Longrightarrow> set (rflts [q]) \<subseteq> U"
    and norm: "\<And>xs c q p. set xs \<subseteq> U \<Longrightarrow>
      q \<in> set xs \<Longrightarrow> p \<in> set (rpder_norm_list c q) \<Longrightarrow>
...
proof (rule rsizes_rpders_strong_rows_finite_universe_boundI
  ...
  assume xs: "set xs \<subseteq> U"
  show "set (rpder_strong_rows c xs) \<subseteq> U"
    by (rule rpder_strong_rows_norm_later_shared_subsetI)
      (use xs flat_closed norm shared in blast)+
qed

lemma rsizes_rpders_strong_rows_norm_later_shared_cubic_universe_boundI:
  assumes init: "set rs \<subseteq> U"
    and flat_closed: "\<And>q. q \<in> U \<Longrightarrow> set (rflts [q]) \<subseteq> U"
    and norm: "\<And>xs c q p. set xs \<subseteq> U \<Longrightarrow>
      q \<in> set xs \<Longrightarrow> p \<in> set (rpder_norm_list c q) \<Longrightarrow>
...
proof -
  have "rsizes (rpders_strong_rows rs s) \<le> card U * M"
    by (rule rsizes_rpders_strong_rows_norm_later_shared_finite_universe_boundI
        [OF init flat_closed norm shared finite member_size distinct])
  also have "... \<le> C * M"
    by (rule mult_right_mono[OF card_bound]) simp
  also have "... \<le> B"
...
lemma rsizes_rpders_strong1_rows_norm_later_shared_finite_universe_boundI:
  assumes init: "r \<in> U"
    and flat_closed: "\<And>q. q \<in> U \<Longrightarrow> set (rflts [q]) \<subseteq> U"
    and norm: "\<And>xs c q p. set xs \<subseteq> U \<Longrightarrow>
      q \<in> set xs \<Longrightarrow> p \<in> set (rpder_norm_list c q) \<Longrightarrow>
...
proof -
  ...
  have "rsizes (rpders_strong_rows [r] s) \<le> card U * M"
    by (rule rsizes_rpders_strong_rows_norm_later_shared_finite_universe_boundI
        [OF rows flat_closed norm shared finite member_size distinct])
  then show ?thesis
    by (simp add: rpders_strong1_rows_def)
qed

lemma rsizes_rpders_strong1_rows_norm_later_shared_cubic_universe_boundI:
  assumes init: "r \<in> U"
    and flat_closed: "\<And>q. q \<in> U \<Longrightarrow> set (rflts [q]) \<subseteq> U"
    and norm: "\<And>xs c q p. set xs \<subseteq> U \<Longrightarrow>
      q \<in> set xs \<Longrightarrow> p \<in> set (rpder_norm_list c q) \<Longrightarrow>
...
proof -
  have "rsizes (rpders_strong1_rows r s) \<le> card U * M"
    by (rule rsizes_rpders_strong1_rows_norm_later_shared_finite_universe_boundI
        [OF init flat_closed norm shared finite member_size])
  also have "... \<le> C * M"
    by (rule mult_right_mono[OF card_bound]) simp
  also have "... \<le> B"
...
lemma card_raw_final_active_suffix_closure_member_pair_budget_bound:
  assumes pair_budget: "raw_final_active_suffix_pair_budget r \<le> P"
    and member_size: "\<And>q. q \<in> raw_final_active_suffix_rows r \<Longrightarrow>
      rsize q \<le> M"
  shows "card (raw_final_active_suffix_closure r) \<le>
    card (raw_final_active_suffix_rows r) + P * M"
proof -
  have "card (raw_shared_prune_active_suffix_closure
      (raw_final_active_suffix_rows r)) \<le>
      card (raw_final_active_suffix_rows r) + P * M"
  proof (rule card_raw_shared_prune_active_suffix_closure_member_pair_budget_bound)
...
lemma card_raw_final_active_suffix_closure_member_pair_budget_card_bound:
  assumes rows_bound: "card (raw_final_active_suffix_rows r) \<le> C"
    and pair_budget: "raw_final_active_suffix_pair_budget r \<le> P"
    and member_size: "\<And>q. q \<in> raw_final_active_suffix_rows r \<Longrightarrow>
      rsize q \<le> M"
  shows "card (raw_final_active_suffix_closure r) \<le> C + P * M"
proof -
  have closure_bound:
...
proof -
  ...
  show ?thesis
    by (rule card_raw_final_active_suffix_closure_member_pair_budget_card_bound
