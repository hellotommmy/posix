...
\large the proved chain, the single open lemma, and exactly why it resists}
...
\[
\sig(\zero,k)=\zero,\quad \sig(\one,k)=k,\quad \sig(r_1\seq r_2,k)=\sig\!\bigl(r_1,\sig(r_2,k)\bigr),\quad
\sig(a,\zero)=\zero,\ \sig(a,\one)=a,\ \sig(a,k)=a\seq k.
\]
...
\item[$\dlf(x)$ \rm(open one regex to linear forms; ``row\_dlforms'')]
\[
\dlf(\zero)=\emptyset,\quad \dlf(\textstyle\sum rs)=\textstyle\bigcup_{q}\dlf(q),\quad
\dlf\bigl((\textstyle\sum ps)\seq k\bigr)=\textstyle\bigcup_{p\in ps}\dlf(\sigt(p,k)),\quad \dlf(r)=\dpar r\ \ (\text{otherwise}).
\]
...
\item[$\Front(r,s)$ \rm(normalised front; ``afactored1'')] read the whole string $s$ with \emph{no} $\Strong$: starting
from $[r]$, apply for each letter $c$ the normalised step $\nu_c(\rho):=\mathrm{dedup}\bigl(\mathrm{flat}\bigcup_{q\in\rho}\Dch_c(q)\bigr)$,
\[
\Front(r,\eps)=[r],\qquad \Front(r,\,c\!:\!s)=\Front\bigl(\text{root of }\nu_c([r])\ \text{etc.}\bigr)\ \ =\ \ \nu_{c_n}\!\circ\cdots\circ\nu_{c_1}([r]).
\]
...
\item[$\Prune(\rho)$ \rm(strong prune; ``rsimpStrong\_prune\_rows\_raw'')] scan $\rho$ left-to-right keeping the pruned rows
seen so far, replacing each row by its pruning against all earlier ones (rows are \emph{shrunk, never dropped}). The only
non-trivial pairwise rule fires on two rows \emph{sharing a right factor} $k$:
$\bigl((\textstyle\sum lrs)\seq k,\ (\textstyle\sum rrs)\seq k\bigr)\ \mapsto\ \sigt\bigl(\textstyle\sum(rrs\setminus lrs),\,k\bigr)$
--- delete from the later row's head-alternation the branches already present in the earlier one ($rrs\setminus lrs$);
every other pair leaves the later row unchanged. Language-preserving and size-non-increasing.
...
$\#\mathcal D(r)\le\rs r+2$ (linear) and each member has size $\le(\rs r+2)^2$ (quadratic) --- the linear$\times$quadratic
...
\[
\mathrm{at}(\zero,xs)=[\mathrm{AU}],\quad \mathrm{at}(\one,xs)=[\mathrm{AU},\mathrm{AO}\,xs,\mathrm{AF}\,xs],\quad
\mathrm{at}(c,xs)=\mathrm{AO}(c\!:\!xs)\#[\mathrm{AU},\mathrm{AO}\,xs,\mathrm{AF}\,xs],
\]
\[
\mathrm{at}(\textstyle\sum rs,xs)=\mathrm{AU}\#\mathrm{AO}(\textstyle\sum rs\!:\!xs)\#\textstyle\bigoplus_q\mathrm{at}(q,xs),\quad
\mathrm{at}(pq,xs)=\mathrm{at}(p,\,q\!:\!xs)\oplus\mathrm{at}(q,xs),\quad
...
\section{The proved chain (top to bottom)}\label{sec:chain}

All machine-checked, no \texttt{sorry}. Each line is justified by the one below it.
...
\section{The single open lemma}\label{sec:open}

The entire Gate is proved \emph{modulo one scalar inequality} --- the intercept of step 8, the \emph{anchor}:
\[
\boxed{\ \pot(r^{*},\,\text{char }c)\ \le\ M^3+3M^2\ }
\qquad\Bigl(=\ \textstyle\sum_{e\in\tr(r)}\wt_{\,\text{char }c}(e)\ \le\ M^3+3M^2\Bigr),\quad M=1+\rs r.
\tag{$\star$}
\]
\OPEN. Machine-validated true: $0$ violations over an \emph{exhaustive} enumeration of all in-regime terms up to
...
$(\text{root }r^{*},\ \text{sub-term }q,\ \text{boundary})$ plus a strict \emph{size}-credit, so the per-event opened size
...
fully proved on top of it.
\end{enumerate}

\paragraph{One-line status.} The clean-fragment cubic Gate is proved \emph{except} the single scalar
$\pot(r^{*},\text{char }c)\le M^3+3M^2$ --- true, exhaustively validated, provably beyond structural induction because it
encodes a global size$\times$multiplicity cancellation; the route that fits is the size analogue of the $T+S$ amortised
argument that already cracked the row count.
