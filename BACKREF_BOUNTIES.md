...
| BR-033 | Prove partial-derivative universe cubic bound | 12,000 | 300 | 10 | 12,000 | OPEN | - | GeneralRegexBound.thy:partial_derivative_path_universe,partial_derivative_live_row_universe,rsizes_distinct_path_universe_cubic,rsize_rpd_der_norm_cubic,rsizes_distinct_frontier_universe_cubic | Isabelle:Posix | Replace `card(sizeNregex N)` reasoning with a finite universe generated from subterms/continuations of the original non-backref regex; one-step cubic and frontier-universe cubic accounting are checked; live-path-only rflts closure is refuted by `live_path_universe_misses_flattened_alt_row`, so repeated-state closure must use a row/frontier invariant |
