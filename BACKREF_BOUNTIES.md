...
| BR-040 | Prove memo-strong-tree POSIX and cubic interface | 8,000 | 180 | 9 | 8,000 | OPEN | - | FBound.thy:strong_deferred_span_value_iff_lexer,strong_deferred_original_memo_budget,strong_deferred_original_raw_row_norm_closed_memo_cubic_interface;GeneralRegexBound.thy:raw_shared_prune_closed,rsizes_rpders_strong1_rows_raw_norm_closed_cubic_universe_boundI | Isabelle:Posix | Prove the checked handoff from the smoke-tested `StrongDeferredMemo` route to a concrete cubic raw/shared universe. Already checked: POSIX/lexer equivalence, nullable recognition gate, quadratic memo-state budget, cubic split-probe budget, and conditional row-universe interface. Still open: instantiate the finite POSIX-safe universe `U` with closure and cubic cardinality/member-size bounds. |
