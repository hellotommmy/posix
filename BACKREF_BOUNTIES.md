...
| BR-040 | Prove memo-strong-tree POSIX and cubic interface | 8,000 | 180 | 9 | 8,000 | OPEN | - | FBound.thy:strong_deferred_span_value_iff_lexer,strong_deferred_memo_exact_value_budget,strong_deferred_original_memo_budget,strong_deferred_original_raw_row_norm_closed_memo_cubic_interface,strong_deferred_original_sizeNregex_memo_cubic_interface;GeneralRegexBound.thy:rflts_sizeNregex_closed,raw_shared_prune_closed,path9_atom_frontier_not_raw_shared_prune_closed,carry9_atom_frontier_not_raw_shared_prune_closed,rsizes_rpders_strong1_rows_raw_norm_closed_cubic_universe_boundI | Isabelle:Posix | Prove the checked handoff from the smoke-tested `StrongDeferredMemo` route to a concrete cubic raw/shared universe. Already checked: exact POSIX value extraction from the strong nullable gate plus span/memo table, POSIX/lexer equivalence, nullable recognition gate, quadratic memo-state budget, cubic split-probe budget, conditional row-universe interface, and a concrete `sizeNregex N` finite-universe fallback. Also checked-false: directly reusing current path9/carry9 atom-frontier universes for raw strong shared-prune closure. Still open: replace the huge `sizeNregex N` fallback with a finite POSIX-safe row-difference/memo frontier universe whose closure and cardinality/member-size bound are genuinely cubic. |
