...
| BR-039 | Define smoke-tested memo-strong-tree candidate | 25,000 | 260 | 10 | 25,000 | OPEN | - | agent_hunt_pipeline/scala/PosixCubicSmoke.scala:StrongDeferredMemo;agent_hunt_pipeline/scripts/scala_cubic_smoke.ps1;FBound.thy:strong_deferred_span_value,strong_deferred_span_value_THE_lexer,strong_deferred_memo_budget,strong_deferred_original_raw_row_norm_closed_memo_cubic_interface | ScalaSmoke+Isabelle:Posix | Main route is now `bsimpStrong` as the small recognition tree plus original-regex span/memo POSIX value reconstruction. Default smoke route is `strong-memo`, which checks exact POSIX values and Chapter 7 memo traces. `bsimpCubic` is retained only as historical/negative diagnostic evidence unless a future variant beats the thesis baseline and preserves POSIX values. |
| BR-040 | Prove memo-strong-tree POSIX and cubic interface | 8,000 | 180 | 9 | 8,000 | OPEN | - | FBound.thy:strong_deferred_span_value_iff_lexer,strong_deferred_memo_exact_value_budget,strong_deferred_original_memo_budget,strong_deferred_original_raw_row_norm_closed_memo_cubic_interface,strong_deferred_original_sizeNregex_memo_cubic_interface;GeneralRegexBound.thy:rflts_sizeNregex_closed,raw_shared_prune_closed,raw_shared_prune_pair_closure,path9_atom_frontier_not_raw_shared_prune_closed,carry9_atom_frontier_not_raw_shared_prune_closed,raw_shared_prune_bad_result_in_path9_pair_closure,raw_shared_prune_bad_result_in_carry9_pair_closure,rsizes_rpders_strong1_rows_raw_norm_closed_cubic_universe_boundI | Isabelle:Posix | Prove the checked handoff from the smoke-tested `StrongDeferredMemo` route to a concrete cubic raw/shared universe. Already checked: exact POSIX value extraction from the strong nullable gate plus span/memo table, POSIX/lexer equivalence, nullable recognition gate, quadratic memo-state budget, cubic split-probe budget, conditional row-universe interface, and a concrete `sizeNregex N` finite-universe fallback. Also checked-false: directly reusing current path9/carry9 atom-frontier universes for raw strong shared-prune closure; one-step pair closure captures the missing same-suffix row-difference witness. Still open: replace the huge `sizeNregex N` fallback with a finite POSIX-safe row-difference/memo frontier universe whose closure and cardinality/member-size bound are genuinely cubic. |
