...
| BR-036 | Close rsimp8 live-row cubic closure | 15,000 | 320 | 10 | 15,000 | OPEN | - | GeneralRegexBound.thy:rsimp8_live_row_universe_not_closed,norm18_closes_rsimp8_live_row_obstruction,rsimp8_live_row_universe_RNTIMES_not_closed,norm18_live_row_NTIMES_body_normalized_sanity,rpder_norm8_list,rpder_norm8_rows,rpders_norm18_rows,RLS_rpders_norm18_rows,rsizes_rpders_norm18_rows_rsimp8_live_row_cubicI,rpder_norm8_rsimp8_live_row_step | Isabelle:Posix | Current `norm17` closure is refuted by `(((1+a).a))*`; checked `norm18 = map rsimp8` repairs that obstruction but the plain target is now refuted for `RNTIMES` because `rsimp8` does not recurse under repetition bodies. The new positive sanity lemma shows closure when the repeated body is already normalized, suggesting a conservative `RNTIMES` body/tail normalization repair. Remaining payout requires a refined invariant or revised root-safe theorem w.r.t. original `rsize r`; non-backref fragment only |
