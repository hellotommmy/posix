...
| Fable | Worker | 0 | Registered 2026-06-12; racing OPEN cubic bounties (BR-033) and the 20k cubic overlay; zero balance so no locks - first-to-complete rules apply. BR-039/BR-040 are FROZEN (status BLOCKED) as of 2026-06-12, not raceable |
...
- Row-DAG decomposition bridge: `GeneralRegexBound.thy` now checks raw
  ...
  A sharper shared-universe handoff
  `card_strong_deferred_final_active_suffix_row_dag_universe_shared_component_boundI`
  shows that one universe covering both payload/key DAG components is enough
  for `finalRowDag <= 2*rows + card U`.
...
- Empty final-active budget bridge: `GeneralRegexBound.thy` and `FBound.thy`
  ...
  metrics are zero. `FBound.thy` also checks
  `strong_deferred_original_final_active_empty_rows_contract`, packaging this
  case with exact POSIX reconstruction and span/split memo budgets. This
  records the route decision after the graphs: emitted-tree `bsimpCubic` is
  negative evidence, and the active target is memo strong tree plus exact POSIX
  reconstruction and final-active row-DAG accounting. This is infrastructure
  only; no bounty is claimed.
