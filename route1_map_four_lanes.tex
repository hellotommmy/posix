...
BND & \code{boundary\_term\_absorb} via \textbf{S1} &
Feeds the \emph{RSEQ recurrence}. Method: the plain subset is false for $\code{RSEQ}$-root; use the asymmetric
split (RSEQ$\to$$\Sstrong$-image $+$ \code{card\_image\_le}; non-RSEQ$\to$plain $+$ \code{card\_mono}). \\
