...
lemma rsimp4_SEQ_atom_nonunit_rtail_nf:
  assumes p_nf: "rtail_nf p"
    and p0: "p \<noteq> RZERO"
    and p1: "p \<noteq> RONE"
    and k0: "k \<noteq> RZERO"
    and k1: "k \<noteq> RONE"
  shows "rsimp4_SEQ_atom p k \<noteq> RZERO \<and>
    rsimp4_SEQ_atom p k \<noteq> RONE"
...
next
  ...
  have head_nonunit:
      "rsimp4_SEQ_atom p1 (rsimp4_SEQ_atom p2 k) \<noteq> RZERO \<and>
       rsimp4_SEQ_atom p1 (rsimp4_SEQ_atom p2 k) \<noteq> RONE"
...
lemma rsimp4_SEQ_atom_nonstar_rtail_nf_not_RSTAR:
  assumes p_nf: "rtail_nf p"
    and p0: "p \<noteq> RZERO"
    and p1: "p \<noteq> RONE"
    and no_star: "\<And>s. p \<noteq> RSTAR s"
    and K_nf: "rtail_nf K"
  shows "rsimp4_SEQ_atom p K \<noteq> RSTAR s"
proof (cases K)
  case RZERO
...
next
  ...
  have stable: "rsimp4_SEQ_atom p RONE = p"
    by (rule rtail_nf_RONE_stable[OF p_nf])
  then show ?thesis
    using RONE no_star[of s] by simp
qed (use p_nf p0 p1 in
    \<open>auto dest: rsimp4_SEQ_atom_nonunit_rtail_nf_not_RSTAR\<close>)

lemma row_dlforms_rsimp7_assoc_RSEQ_left_nonunit_tail_subset:
  assumes a_def: "a = RSEQ a1 a2"
    and a_nf: "rtail_nf a"
    and k_nf: "rtail_nf k"
    and k0: "k \<noteq> RZERO"
    and k1: "k \<noteq> RONE"
  shows "row_dlforms (rsimp7_SEQ_atom (rsimp7_SEQ_atom a k) K) \<subseteq>
    row_dlforms (rsimp7_SEQ_atom a (rsimp4_SEQ_atom k K))"
proof -
  have a0: "a \<noteq> RZERO"
    using a_def by simp
  have a1_ne: "a \<noteq> RONE"
    using a_def by simp
  have mid_not_star:
      "\<And>s. rsimp4_SEQ_atom a k \<noteq> RSTAR s"
    by (rule rsimp4_SEQ_atom_nonunit_rtail_nf_not_RSTAR
        [OF a_nf a0 a1_ne k0 k1])
  have first_eq:
      "rsimp7_SEQ_atom a k = rsimp4_SEQ_atom a k"
    using a_def by (simp add: rsimp7_SEQ_atom_def)
  have second_eq:
      "rsimp7_SEQ_atom (rsimp4_SEQ_atom a k) K =
       rsimp4_SEQ_atom (rsimp4_SEQ_atom a k) K"
  proof (cases "rsimp4_SEQ_atom a k")
    case (RSTAR s)
    then show ?thesis
      using mid_not_star[of s] by simp
  qed (simp_all add: rsimp7_SEQ_atom_def)
  have right_eq:
      "rsimp7_SEQ_atom a (rsimp4_SEQ_atom k K) =
       rsimp4_SEQ_atom a (rsimp4_SEQ_atom k K)"
    using a_def by (simp add: rsimp7_SEQ_atom_def)
  show ?thesis
    by (simp add: first_eq second_eq right_eq rsimp4_SEQ_atom_assoc)
qed

lemma row_dlforms_rsimp7_assoc_nonstar_tail_subset:
  assumes a_nf: "rtail_nf a"
    and k_nf: "rtail_nf k"
    and k0: "k \<noteq> RZERO"
    and k1: "k \<noteq> RONE"
    and no_star: "\<And>s. k \<noteq> RSTAR s"
    and K_nf: "rtail_nf K"
  shows "row_dlforms (rsimp7_SEQ_atom (rsimp7_SEQ_atom a k) K) \<subseteq>
    row_dlforms (rsimp7_SEQ_atom a (rsimp4_SEQ_atom k K))"
proof (cases k)
  ...
  have sub: "row_dlforms
    ...
    by (rule row_dlforms_rsimp7_assoc_RSEQ_tail_subset
        [OF a_nf _ K_nf])
      (use RSEQ k_nf in simp)
  then show ?thesis
    using RSEQ sub by simp
next
  case (RSTAR s)
  then show ?thesis
    using no_star[of s] by simp
next
  case RZERO
  then show ?thesis
    using k0 by simp
...
next
  ...
  next
    case (RSEQ a1 a2)
    show ?thesis
      by (rule row_dlforms_rsimp7_assoc_RSEQ_left_nonunit_tail_subset
          [OF RSEQ a_nf k_nf k0 k1])
  qed (use RALTS in
      \<open>simp_all add: rsimp7_SEQ_atom_def rsimp4_SEQ_atom_assoc\<close>)
next
  case (RCHAR c)
...
lemma row_dlforms_rsimp7_nested_altseq_nonstar_tail_subset_later:
  assumes ps: "set ps \<subseteq> set qs"
    and qs_props: "\<forall>q \<in> set qs.
      rtail_nf q \<and> nonalt q \<and> q \<noteq> RZERO"
  ...
  shows "row_dlforms
      (rsimp7_SEQ_atom
        (rsimp7_SEQ_atom (rsimp_ALTs ps) tail) K) \<subseteq>
    row_dlforms (rsimp7_SEQ_atom (RSEQ (RALTS qs) tail) K)"
proof -
  have qs_nf: "\<forall>q \<in> set qs. rtail_nf q"
    using qs_props by blast
  have ps_props: "\<forall>p \<in> set ps.
  ...
  have assoc:
      "row_dlforms
       ...
       row_dlforms
        (rsimp7_SEQ_atom (rsimp_ALTs ps)
          (rsimp4_SEQ_atom tail K))"
    by (rule row_dlforms_rsimp7_assoc_nonstar_tail_subset
        [OF a_nf tail_nf tail0 tail1 no_star K_nf])
  have branch:
      "row_dlforms
        (rsimp7_SEQ_atom (rsimp_ALTs ps)
          (rsimp4_SEQ_atom tail K)) \<subseteq>
       row_dlforms
        (rsimp7_SEQ_atom (RALTS qs)
          (rsimp4_SEQ_atom tail K))"
    by (rule row_dlforms_rsimp7_rsimp_ALTs_subset_RALTS
...
lemma row_dlforms_rsimp7_prune_pair_shared_nonstar_tail_subset_later:
...
next
  ...
  have nested:
      "row_dlforms
        (rsimp7_SEQ_atom
          (rsimp7_SEQ_atom
            (rsimp_ALTs (rprune_eq_against lrs rrs)) tail) K) \<subseteq>
       row_dlforms (rsimp7_SEQ_atom (RSEQ (RALTS rrs) tail) K)"
    by (rule row_dlforms_rsimp7_nested_altseq_nonstar_tail_subset_later
        [OF pruned rrs_props tail_nf tail0 tail1 no_star K_nf])
  show ?thesis
    using earlier later nonempty nested
    by (simp add: rsimpStrong_prune_pair_raw_def)
qed
...
lemma singleton_saa_ok_prune_pair_raw_shared_nonstar_if_ok:
  assumes earlier: "earlier = RSEQ (RALTS lrs) tail"
    and later: "later = RSEQ (RALTS rrs) tail"
    and ok: "singleton_saa_ok q later"
    and rrs_props: "\<forall>r \<in> set rrs.
...
proof
  ...
  have split:
      "row_dlforms
       ...
       row_dlforms (rsimp7_SEQ_atom later (rsimpStrong_raw k))"
    by (rule row_dlforms_rsimp7_prune_pair_shared_nonstar_tail_subset_later
        [OF earlier later rrs_props tail_nf tail0 tail1 no_star K_nf])
  have later_cover:
      "row_dlforms (rsimp7_SEQ_atom later (rsimpStrong_raw k)) \<subseteq>
       strong_apder_acc (RALTS [q]) k"
    using ok unfolding singleton_saa_ok_def by blast
...
lemma tagged_prune_pair_raw_preserves_singleton_saa_scan_ok_shared_nonstar_if_tail_key:
  assumes scan: "singleton_saa_scan_ok (fst later) (snd later)"
    and earlier: "snd earlier = RSEQ (RALTS lrs) tail"
    and later: "snd later = RSEQ (RALTS rrs) tail"
    and later_nf: "rtail_nf (snd later)"
    and no_star: "\<And>s. tail \<noteq> RSTAR s"
    and tail_key: "singleton_saa_key_credit (fst later) tail"
  shows "singleton_saa_scan_ok
    (fst (tagged_prune_pair_raw earlier later))
    (snd (tagged_prune_pair_raw earlier later))"
proof -
  ...
  have pruned_scan:
      "singleton_saa_scan_ok (fst later)
        (rsimpStrong_prune_pair_raw (snd earlier) (snd later))"
    by (rule singleton_saa_scan_ok_prune_pair_raw_shared_nonstar_if_tail_key
        [OF earlier later scan rrs_props tail_nf tail0 tail1 no_star
          tail_key])
  show ?thesis
    using pruned_scan by (simp add: tagged_prune_pair_raw_def)
qed
